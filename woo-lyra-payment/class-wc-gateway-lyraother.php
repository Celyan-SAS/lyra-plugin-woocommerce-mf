...
class WC_Gateway_LyraOther extends WC_Gateway_LyraStd
{
    ...
    public function __construct($payment_code, $payment_title)
    {
        ...
        $this->regrouped_other_payments = new WC_Gateway_LyraRegroupedOther(false);

        $code = strtolower($this->payment_code);
        $this->id = 'lyraother_' . $code;
    ...
    public function get_title()
    {
        ...
        if (! $title) {
            ...
            $title = sprintf(__('Payment with %s', 'woo-lyra-payment'), $cards[$this->payment_code]);
        }
    ...
    protected function lyra_fill_request($order)
    {
        ...
        // Set payment card.
        $this->lyra_request->set('payment_cards', $this->payment_code);
