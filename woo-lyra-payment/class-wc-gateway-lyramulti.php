...
class WC_Gateway_LyraMulti extends WC_Gateway_LyraStd
{
    public function __construct()
    {
        ...
        $this->id = 'lyramulti';
        $this->icon = apply_filters('woocommerce_lyramulti_icon', WC_LYRA_PLUGIN_URL . 'assets/images/lyramulti.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Payment in installments', 'woo-lyra-payment');
        ...
        if ($lyra_plugin_features['restrictmulti']) {
            $this->notices[] = __('ATTENTION: The payment in installments feature activation is subject to the prior agreement of Société Générale.<br />If you enable this feature while you have not the associated option, an error 10000 – INSTALLMENTS_NOT_ALLOWED or 07 - PAYMENT_CONFIG will occur and the buyer will not be able to pay.', 'woo-lyra-payment');
        }
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables multiple payment.', 'woo-lyra-payment');

        $this->form_fields['title']['default'] = __('Payment by credit card in installments', 'woo-lyra-payment');
        ...
        $this->form_fields['card_data_mode'] = array(
            'title' => __('Card type selection', 'woo-lyra-payment'),
            'type' => 'select',
            'default' => 'DEFAULT',
            'options' => array(
                'DEFAULT' => __('On payment gateway', 'woo-lyra-payment'),
                'MERCHANT' => __('On merchant site', 'woo-lyra-payment')
            ),
            'description' =>sprintf(__('Select where card type will be selected by buyer.', 'woo-lyra-payment'), self::GATEWAY_NAME),
            'class' => 'wc-enhanced-select'
        );

        $this->form_fields['multi_options'] = array(
            'title' => __('MULTIPLE PAYMENT OPTIONS', 'woo-lyra-payment'),
            'type' => 'title'
        );

        // Multiple payment options.
        $descr = __('Click on "Add" button to configure one or more payment options.<br /><b>Label: </b>The option label to display on the frontend.<br /><b>Min amount: </b>Minimum amount to enable the payment option.<br /><b>Max amount: </b>Maximum amount to enable the payment option.<br /><b>Count: </b>Total number of payments.<br /><b>Period: </b>Delay (in days) between payments.<br /><b>1st payment: </b>Amount of first payment, in percentage of total amount. If empty, all payments will have the same amount.<br /><b>Do not forget to click on "Save" button to save your modifications.</b>', 'woo-lyra-payment');

        $cards = $this->get_supported_card_types();

        $columns = array();
        $columns['label'] = array(
            'title' => __('Label', 'woo-lyra-payment'),
            'width' => '154px'
        );

        $columns['amount_min'] = array(
            'title' => __('Min amount', 'woo-lyra-payment'),
            'width' => '92px'
        );

        $columns['amount_max'] = array(
            'title' => __('Max amount', 'woo-lyra-payment'),
            'width' => '92px'
        );

        if (isset($cards['CB'])) {
            $columns['contract'] = array(
                'title' => __('Contract', 'woo-lyra-payment'),
                'width' => '74px'
            );

            $descr = __('Click on "Add" button to configure one or more payment options.<br /><b>Label: </b>The option label to display on the frontend.<br /><b>Min amount: </b>Minimum amount to enable the payment option.<br /><b>Max amount: </b>Maximum amount to enable the payment option.<br /><b>Contract: </b>ID of the contract to use with the option (leave blank preferably).<br /><b>Count: </b>Total number of payments.<br /><b>Period: </b>Delay (in days) between payments.<br /><b>1st payment: </b>Amount of first payment, in percentage of total amount. If empty, all payments will have the same amount.<br /><b>Do not forget to click on "Save" button to save your modifications.</b>', 'woo-lyra-payment');
        }

        $columns['count'] = array(
            'title' => __('Count', 'woo-lyra-payment'),
            'width' => '74px'
        );

        $columns['period'] = array(
            'title' => __('Period', 'woo-lyra-payment'),
            'width' => '74px'
        );

        $columns['first'] = array(
            'title' => __('1st payment', 'woo-lyra-payment'),
            'width' => '84px'
        );

        $this->form_fields['payment_options'] = array(
            'title' => __('Payment options', 'woo-lyra-payment'),
            'type' => 'table',
            'columns' => $columns,
            'description' => $descr
        );
...
?>
        ...
        //<!--
            function lyraAddOption(fieldName, record, key) {
                ...
                optionLine += '<td style="padding: 0px;"><input type="button" value="<?php echo __('Delete', 'woo-lyra-payment')?>" onclick="javascript: lyraDeleteOption(\'' + fieldName + '\', \'' + key + '\');"></td>';
            ...
            function lyraDeleteOption(fieldName, key) {
...
<?php
    ...
    private function get_available_options()
    {
        ...
        if (is_admin()) {
            ...
            return apply_filters("woocommerce_lyramulti_enabled_options", $enabled_options);
        }
    ...
    public function get_payment_fields($options = array())
    {
        ...
        if (count($options) == 1) {
            ...
            $html .= '<span style="font-weight: bold;">' . __('Your payment option', 'woo-lyra-payment') . '</span>';
            $html .= '<li style="list-style-type: none;">
                    <input type="hidden" id="lyramulti_option_' . $key . '" value="' . $key . '" name="lyramulti_option">
                    <label style="display: inline;">' . $option['label'] . '</label>
                  </li>';
        } else {
            $first = true;
            $html .= '<span style="font-weight: bold;">' . __('Choose your payment option', 'woo-lyra-payment') . '</span>';
            foreach ($options as $key => $option) {
                $html .= '<li style="list-style-type: none;">
                        <input class="radio" type="radio"' . ($first == true ? ' checked="checked"' : '') . ' id="lyramulti_option_' . $key . '" value="' . $key . '" name="lyramulti_option">
                        <label for="lyramulti_option_' . $key . '" style="display: inline;">' . $option['label'] . '</label>
                      </li>';
                $first = false;
            }
        }
    ...
    public function process_payment($order_id)
    {
        ...
        $options = $this->get_available_options();
        $option_id = isset($_POST['lyramulti_option']) ? $_POST['lyramulti_option'] : $_COOKIE['lyramulti_option'];
        $option = $options[$option_id];

        // Save selected payment option into session...
        set_transient('lyramulti_option_' . $order_id, $option);
    ...
    protected function lyra_fill_request($order)
    {
        ...
        $option = get_transient('lyramulti_option_' . self::get_order_property($order, 'id'));

        // Multiple payment options.
        $amount = $this->lyra_request->get('amount');
        $first = $option['first'] ? round(($option['first'] / 100) * $amount) : null;
        $this->lyra_request->setMultiPayment($amount, $first, $option['count'], $option['period']);
        $this->lyra_request->set('contracts', (isset($option['contract']) && $option['contract']) ? 'CB=' . $option['contract'] : null);

        delete_transient('lyramulti_option_' . self::get_order_property($order, 'id'));
    }
}
