...
class WC_Gateway_LyraFranfinance extends WC_Gateway_LyraStd
{
    protected $lyra_countries = array('FR', 'GP', 'MQ', 'GF', 'RE', 'YT'); // France and DOM.
    protected $lyra_currencies = array('EUR');

    public function __construct()
    {
        $this->id = 'lyrafranfinance';
        $this->icon = apply_filters('woocommerce_lyrafranfinance_icon', WC_LYRA_PLUGIN_URL . 'assets/images/franfinance.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Franfinance payment', 'woo-lyra-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Franfinance payment.', 'woo-lyra-payment');

        $this->form_fields['title']['default'] = __('Payment with Franfinance', 'woo-lyra-payment');
        ...
        $this->form_fields['payment_options'] = array(
            'title' => __('FRANFINANCE PAYMENT OPTIONS', 'woo-lyra-payment'),
            'type' => 'title'
        );
        ...
        $columns['fees'] = array(
            'title' => __('Fees', 'woo-lyra-payment'),
            'default' => '-1',
            'width' => '125px'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'columns' => $columns,
            'default' => array (
                '3x' => array('label' => sprintf(__('Payment in %s times', 'woo-lyra-payment'), '3'), 'count' => '3', 'fees' => '-1', 'amount_min' => '100', 'amount_max' => '3000'),
                '4x' => array('label' => sprintf(__('Payment in %s times', 'woo-lyra-payment'), '4'), 'count' => '4', 'fees' => '-1', 'amount_min' => '100', 'amount_max' => '4000'),
            ),
            ...
            function lyraAddOption(fieldName, record, key) {
                ...
                jQuery.each(orderedRecord, function(attr, value) {
                    ...
                    switch (attr) {
                        ...
                        case 'fees':
                            optionLine += '<select style="width: ' + width + 'px;" name="' + inputName + '" id="' + inputName + '">';
                            optionLine += '<?php $options = array('-1' => sprintf(__('%s Back Office configuration', 'woo-lyra-payment'), self::BACKOFFICE_NAME), 'N' => __('Without fees', 'woo-lyra-payment'), 'Y' => __('With fees', 'woo-lyra-payment'));
                                                foreach ($options as $key => $value) {
                                                    echo '<option value="' . $key . '">' . $value . '</option>';
                                                } ?>';
...
<?php
    ...
    public function validate_payment_options_field($key, $value = null)
    {
        ...
        foreach ($value as $code => $option) {
            ...
            if (! $option['label']) {
                $value[$code]['label'] = sprintf(__('Payment in %s times', 'woo-lyra-payment'), $option['count']);
            }
        }
    ...
    public function get_payment_fields($options = array())
    {
        ...
        if (count($options) == 1) {
            ...
            $html .= '<li style="list-style-type: none;">
                    <input type="hidden" id="lyrafranfinance_option_' . $key . '" value="' . $key . '" name="lyrafranfinance_option">
            ...
            foreach ($options as $key => $option) {
                $html .= '<li style="list-style-type: none;">
                        <input class="radio" type="radio"' . ($first ? ' checked="checked"' : '') . ' id="lyrafranfinance_option_' . $key . '" value="' . $key . '" name="lyrafranfinance_option">
                        <label for="lyrafranfinance_option_' . $key . '" style="display: inline;">' . $option['label'] . '</label>
                      </li>';
                $first = false;
            }
        }
    ...
    public function process_payment($order_id)
    {
        ...
        $options = $this->get_available_options();
        $option_id = isset($_POST['lyrafranfinance_option']) ? $_POST['lyrafranfinance_option'] : $_COOKIE['lyrafranfinance_option'];
        $option = $options[$option_id];

        // Save selected payment option into session...
        set_transient('lyrafranfinance_option_' . $order_id, $option);
    ...
    protected function lyra_fill_request($order)
    {
        ...
        $this->lyra_request->set('validation_mode', '0');
        $this->lyra_request->set('capture_delay', '0');

        $option = get_transient('lyrafranfinance_option_' . self::get_order_property($order, 'id'));
        $this->lyra_request->set('payment_cards', 'FRANFINANCE_' . $option['count'] . 'X');

        if ($option['fees'] !== '-1') {
            $fees = $option['fees'] ? 'Y' : 'N';
            $this->lyra_request->set('acquirer_transient_data', '{"FRANFINANCE":{"FEES_' . $option['count'] . 'X":"' . $fees . '"}}');
        }

        delete_transient('lyrafranfinance_option_' . self::get_order_property($order, 'id'));
    }
}
