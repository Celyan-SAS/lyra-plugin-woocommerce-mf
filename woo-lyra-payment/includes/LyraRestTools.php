...
class LyraRestTools
{
    ...
    public static function checkResult($response, $expectedStatuses = array())
    {
        ...
        if ($response['status'] != 'SUCCESS') {
            ...
            require_once 'LyraRestException.php';
            throw new LyraRestException($errorMessage, $answer['errorCode']);
        } elseif (! empty($expectedStatuses) && ! in_array($answer['detailedStatus'], $expectedStatuses)) {
            throw new Exception(sprintf(__('Unexpected transaction type received (%1$s).', 'woo-lyra-payment'), $answer['detailedStatus']));
        }
    }
}
