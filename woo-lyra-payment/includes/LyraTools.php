...
class LyraTools
{
    public static function get_contrib()
    {
        ...
        return WC_Gateway_Lyra::CMS_IDENTIFIER . '_' . WC_Gateway_Lyra::PLUGIN_VERSION . '/' . $version
    ...
    {
        $std_method_settings = get_option('woocommerce_lyrastd_settings', null);
    ...
    {
        $std_settings = get_option('woocommerce_lyrastd_settings', null);
    ...
    public static function get_transaction_uuid($order)
    {
        ...
        if ($trans_id) {
            $notes = WC_Gateway_Lyra::get_order_notes($order->get_id());
