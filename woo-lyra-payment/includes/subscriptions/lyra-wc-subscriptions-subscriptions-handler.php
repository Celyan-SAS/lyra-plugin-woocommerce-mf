...
class Lyra_WC_Subscriptions_Subscriptions_Handler implements Lyra_Subscriptions_Handler_Interface
{
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::init_hooks()
     */
    public function init_hooks()
    {
        add_action('woocommerce_subscription_pending-cancel_lyrasubscription', array($this, 'cancel_subscription'));
        add_action('wcs_subscription_schedule_after_billing_schedule', array($this, 'update_subscription'));
    }

    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::cart_contains_subscription()
     */
    public function cart_contains_subscription($cart = null)
    {
        ...
        if ($order = WC_Gateway_LyraStd::order_created_from_bo()) {
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::cart_contains_multiple_subscriptions()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::is_subscription_update()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::get_parent_order()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::subscription_info()
     */
    public function subscription_info($order)
    {
        $order_id = wcs_get_objects_property($order, 'id');

        $old_payment_method = get_transient('lyrasubscription_change_payment_' . $order_id);
        $is_payment_change = $old_payment_method ? true : false;
        delete_transient('lyrasubscription_change_payment_' . $order_id);
        ...
        if ($is_payment_change) {
            ...
            if (($old_payment_method === 'lyrasubscription') && $subscription_meta) {
                return false;
            }
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::process_subscription()
     */
    public function process_subscription($order, $response)
    {
        ...
        if (LyraTools::is_hpos_enabled()) {
            ...
            $subscription->update_meta_data('Subscription amount', WC_Gateway_Lyra::display_amount($response->get('sub_amount'), $response->get('sub_currency')));
            ...
            update_post_meta($subscription->get_id(), 'Subscription amount', WC_Gateway_Lyra::display_amount($response->get('sub_amount'), $response->get('sub_currency')));
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::update_subscription()
     */
    public function process_subscription_renewal($order, $response)
    {
        ...
        if (wcs_get_objects_property($subscription, 'payment_method') !== 'lyrasubscription') {
            ...
            if (method_exists($subscription, 'set_payment_method')) {
                $subscription->set_payment_method($lyra_subscription);
            } else {
                $subscription->payment_method = $lyra_subscription->id;
                $subscription->payment_method_title = $lyra_subscription->get_title();
            }
        ...
        if ($renewal_order->has_status('pending')) {
            try {
                ...
                $order_note = sprintf(_x('IPN subscription payment for recurrence #%s.', 'used in order note', 'woo-lyra-payment'), $recurrence_number);
                ...
                if (LyraTools::is_hpos_enabled()) {
                    ...
                    $renewal_order->update_meta_data('Subscription amount', WC_Gateway_Lyra::display_amount($response->get('amount'), $currency_code));
                    ...
                    update_post_meta($renewal_order_id, 'Subscription amount', WC_Gateway_Lyra::display_amount($response->get('amount'), $currency_code));
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::cancel_subscription()
     */
    public function cancel_subscription()
    {
        ...
        $lyra_subscription = new WC_Gateway_LyraSubscription();
        $lyra_subscription->cancel_online_subscription($subscription_id, $order_id);
    }
    ...
    public function update_subscription()
    {
        ...
        $lyra_subscription = new WC_Gateway_LyraSubscription();
        $lyra_subscription->update_online_subscription($subscription_id, $order_id);
    }
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::get_view_order_url()
     */
    ...
    /**
     * {@inheritDoc}
     * @see Lyra_Subscriptions_Handler_Interface::get_subscription_statuses()
     */
    ...
    private function get_renewal_order($subscription, $recurrence)
    {
        foreach ($subscription->get_related_orders('ids', 'renewal') as $renewal_order_id) {
            ...
            $recurrenceNum = (LyraTools::is_hpos_enabled()) ? $order->get_meta('Recurrence number', true) : get_post_meta($renewal_order_id, 'Recurrence number', true);
    ...
    private function create_renewal_order($subscription)
    {
        ...
        if (is_wp_error($renewal_order)) {
            ...
            if (is_wp_error($renewal_order)) {
                throw new Exception(sprintf( __('Error: Unable to create renewal order with %s plugin.', 'woo-lyra-payment'), WC_Gateway_Lyra::GATEWAY_NAME));
            }
        }
