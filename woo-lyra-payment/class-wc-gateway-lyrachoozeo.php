...
class WC_Gateway_LyraChoozeo extends WC_Gateway_LyraStd
{
    ...
    {
        $this->id = 'lyrachoozeo';
        $this->icon = apply_filters('woocommerce_lyrachoozeo_icon', WC_LYRA_PLUGIN_URL . 'assets/images/choozeo.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME. ' - ' . __('Choozeo payment', 'woo-lyra-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables Choozeo payment.', 'woo-lyra-payment');

        $this->form_fields['title']['default'] = __('Payment with Choozeo without fees', 'woo-lyra-payment');
        ...
        $this->form_fields['multi_options'] = array(
            'title' => __('CHOOZEO PAYMENT OPTIONS', 'woo-lyra-payment'),
            'type' => 'title'
        );
        ...
        $this->form_fields['payment_options'] = array(
            ...
            'description' => __('Define amount restriction for each card.', 'woo-lyra-payment')
        );
    }
    ...
    public function payment_fields()
    {
        ...
        if (count($options) == 1) {
            echo '<span style="font-weight: bold;">' . __('Your payment option', 'woo-lyra-payment') . '</span>';
        } else {
            echo '<span style="font-weight: bold;">' . __('Choose your payment option', 'woo-lyra-payment') . '</span>';
        }
        ...
        foreach ($options as $key => $option) {
            ...
            if (count($options) == 1) {
                echo '<input type="hidden"
                             id="lyrachoozeo_option_' . $key . '"
                             value="' . $key . '"
                             name="lyrachoozeo_option">';
            } else {
            ...
            echo '<label for="lyrachoozeo_option_' . $key . '" style="display: inline;">' . '
    ...
    public function process_payment($order_id)
    {
        ...
        $option = $_POST['lyrachoozeo_option'];
        $options = $this->get_available_options();
        $label = $options[$option]['label'];

        // Save selected payment option into session...
        set_transient('lyrachoozeo_option_' . $order_id, $option);
    ...
    protected function lyra_fill_request($order)
    {
        ...
        $option = get_transient('lyrachoozeo_option_' . self::get_order_property($order, 'id'));
        $this->lyra_request->set('payment_cards', $option);
        ...
        $this->lyra_request->set('cust_country', 'FR');

        delete_transient('lyrachoozeo_option_' . self::get_order_property($order, 'id'));
    }
}
