...
class WC_Gateway_Lyra extends WC_Payment_Gateway
{
    const GATEWAY_CODE = 'Lyra';
    const GATEWAY_NAME = 'Lyra Collect';
    const BACKOFFICE_NAME = 'Lyra Expert';
    const GATEWAY_URL = 'https://secure.lyra.com/vads-payment/';
    const REST_URL = 'https://api.lyra.com/api-payment/';
    const STATIC_URL = 'https://static.lyra.com/static/';
    const LOGO_URL = 'https://secure.lyra.com/static/latest/images/type-carte/';
    ...
    const SUPPORT_EMAIL = 'support-ecommerce@lyra-collect.com';
     ...
     * @var LyraRequest
     */
    protected $lyra_request;

    public function __construct()
    {
        $this->id = 'lyra';
        $this->has_fields = false;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('General configuration', 'woo-lyra-payment');
        ...
        $this->title = __('General configuration', 'woo-lyra-payment');
        $this->enabled = false;
        ...
        add_action('woocommerce_api_wc_gateway_lyra', array($this, 'lyra_notify_response'));

        // Filter to allow order status override.
        add_filter('woocommerce_payment_complete_order_status', array($this, 'lyra_complete_order_status'), 10, 2);

        // Customize email.
        add_action('woocommerce_email_after_order_table', array($this, 'lyra_add_order_email_payment_result'), 10, 3);

        // Print our notices.
        add_action('woocommerce_before_template_part', array($this, 'lyra_notices'), 10, 4);
        add_action('woocommerce_before_thankyou', array($this, 'lyra_thankyou'), 10, 4);
        ...
        add_action('woocommerce_api_wc_gateway_lyra_delete_saved_card', array($this, 'lyra_delete_saved_card'));

        // Delete order on failed payment.
        add_action('woocommerce_order_status_changed', array($this, 'lyra_delete_order_with_status_failed'), 10, 1);
    }

    protected function lyra_is_section_loaded()
    {
    ...
    protected function lyra_init()
    {
        $this->logger = new WC_Logger();

        // Init API.
        $this->lyra_request = new LyraRequest();
    ...
    public function lyra_admin_head_style()
    {
        ?>
        <style>
            .lyra p.description {
            ...
            #woocommerce_lyra_url_check + p.description span.url {
                color: #23282d !important;
                font-size: 16px;
                font-weight: bold;
            }

            #woocommerce_lyra_url_check + p.description span.desc {
                color: red !important;
            }

            #woocommerce_lyra_url_check + p.description img {
                vertical-align: middle;
                margin-right: 5px;
            }

            #woocommerce_lyra_rest_check_url + p.description span.url {
                color: #23282d !important;
                font-size: 16px;
                font-weight: bold;
            }

            #woocommerce_lyra_rest_check_url + p.description span.desc {
                color: red !important;
                display: inline-block;
            }

            #woocommerce_lyra_rest_check_url + p.description img {
                margin-right: 5px;
            }
        </style>
        <?php
    ...
    public function lyra_admin_head_script()
    {
        ?>
        <script type="text/javascript">
        //<!--
            jQuery(function() {
                lyraUpdateCategoryDisplay();
                lyraUpdateShippingOptionsDisplay();
            });

            function lyraUpdateCategoryDisplay() {
    ...
    public function admin_options()
    {
        if (! $this->is_supported_currency()) {
            echo '<div class="inline error"><p><strong>' . __('Gateway disabled', 'woo-lyra-payment') . ': ' . sprintf(__('%s does not support your store currency.', 'woo-lyra-payment'), self::GATEWAY_NAME) . '</strong></p></div>';
        }

        if (get_transient($this->id . '_settings_reset')) {
            delete_transient($this->id . '_settings_reset');

            echo '<div class="inline updated"><p><strong>' . sprintf(__('Your %s module configuration is successfully reset.', 'woo-lyra-payment'), self::GATEWAY_NAME) . '</strong></p></div>';
        }
        ...
        <p><?php echo sprintf(__('The module works by sending users to %s in order to select their payment mean and enter their payment information.', 'woo-lyra-payment'), self::GATEWAY_NAME); ?></p>
        ...
        <a href="<?php echo $this->reset_admin_link; ?>"><?php _e('Reset configuration', 'woo-lyra-payment');?></a>

        <?php
    }

    public function lyra_reset_admin_options()
    {
    ...
    protected function get_supported_languages()
    {
        ...
        foreach (LyraApi::getSupportedLanguages() as $code => $label) {
            $langs[$code] = __($label, 'woo-lyra-payment');
        }
    ...
    {
        $modes = array(
            '-1' => sprintf(__('%s general configuration', 'woo-lyra-payment'), self::GATEWAY_NAME),
            ' ' => sprintf(__('%s Back Office configuration', 'woo-lyra-payment'), self::BACKOFFICE_NAME),
            '0' => __('Automatic', 'woo-lyra-payment'),
            '1' => __('Manual', 'woo-lyra-payment')
        );
    ...
    {
        $categories = array(
            'FOOD_AND_GROCERY' => __('Food and grocery', 'woo-lyra-payment'),
            'AUTOMOTIVE' => __('Automotive', 'woo-lyra-payment'),
            'ENTERTAINMENT' => __('Entertainment', 'woo-lyra-payment'),
            'HOME_AND_GARDEN' => __('Home and garden', 'woo-lyra-payment'),
            'HOME_APPLIANCE' => __('Home appliance', 'woo-lyra-payment'),
            'AUCTION_AND_GROUP_BUYING' => __('Auction and group buying', 'woo-lyra-payment'),
            'FLOWERS_AND_GIFTS' => __('Flowers and gifts', 'woo-lyra-payment'),
            'COMPUTER_AND_SOFTWARE' => __('Computer and software', 'woo-lyra-payment'),
            'HEALTH_AND_BEAUTY' => __('Health and beauty', 'woo-lyra-payment'),
            'SERVICE_FOR_INDIVIDUAL' => __('Service for individual', 'woo-lyra-payment'),
            'SERVICE_FOR_BUSINESS' => __('Service for business', 'woo-lyra-payment'),
            'SPORTS' => __('Sports', 'woo-lyra-payment'),
            'CLOTHING_AND_ACCESSORIES' => __('Clothing and accessories', 'woo-lyra-payment'),
            'TRAVEL' => __('Travel', 'woo-lyra-payment'),
            'HOME_AUDIO_PHOTO_VIDEO' => __('Home audio, photo, video', 'woo-lyra-payment'),
            'TELEPHONY' => __('Telephony', 'woo-lyra-payment')
        );
        ...
        if ($no_select_opt) {
            ...
            return array_merge(
                array(
                    '' => '---',
                    'CUSTOM_MAPPING' => __('(Use category mapping below)', 'woo-lyra-payment'),
                ),
                $categories
            );
        }
    ...
    {
        return __('Method title to display on payment means page.', 'woo-lyra-payment');
    }

    protected function get_method_description_field_description()
    {
        return __('This controls the description which the user sees during checkout.', 'woo-lyra-payment');
    }
    ...
    {
        global $woocommerce, $lyra_plugin_features;
        ...
        $docs = __('Click to view the module configuration documentation: ', 'woo-lyra-payment');
        ...
        $this->form_fields = array(
            ...
            'module_details' => array(
                'title' => __('MODULE DETAILS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'developed_by' => array(
                'title' => __('Developed by', 'woo-lyra-payment'),
                'type' => 'text',
                'description' => '<b><a href="https://www.lyra.com/" target="_blank">Lyra Network</a></b>',
                'css' => 'display: none;'
            ),
            'contact' => array(
                'title' => __('Contact us', 'woo-lyra-payment'),
                'type' => 'text',
                'description' => '<b>' . LyraApi::formatSupportEmails(self::SUPPORT_EMAIL) . '</b>',
                'css' => 'display: none;'
            ),
            'contrib_version' => array(
                'title' => __('Module version', 'woo-lyra-payment'),
                'type' => 'text',
                'description' => self::PLUGIN_VERSION,
                'css' => 'display: none;'
            ),
            'platform_version' => array(
                'title' => __('Gateway version', 'woo-lyra-payment'),
                'type' => 'text',
            ...
            'base_settings' => array(
                'title' => __('BASE SETTINGS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'debug' => array(
                'title' => __('Logs', 'woo-lyra-payment'),
                'label' => __('Enable / disable', 'woo-lyra-payment'),
                'type' => 'checkbox',
                'default' => 'yes',
                'description' => sprintf(__('Enable / disable module logs. The log file will be inside <code>%s</code>.', 'woo-lyra-payment'), $log_folder),
            ),

            // Payment gateway access params.
            'payment_gateway_access' => array(
                'title' => __('PAYMENT GATEWAY ACCESS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'site_id' => array(
                'title' => __('Shop ID', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => self::SITE_ID,
                'description' => sprintf(__('The identifier provided by %s.', 'woo-lyra-payment'), self::GATEWAY_NAME),
                'custom_attributes' => array('autocomplete' => 'off')
            ),
            'key_test' => array(
                'title' => __('Key in test mode', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => self::KEY_TEST,
                'description' => sprintf(__('Key provided by %s for test mode (available in %s Back Office).', 'woo-lyra-payment'), self::GATEWAY_NAME, self::BACKOFFICE_NAME),
                'custom_attributes' => array('autocomplete' => 'off')
            ),
            'key_prod' => array(
                'title' => __('Key in production mode', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => self::KEY_PROD,
                'description' => sprintf(__('Key provided by %s (available in %s Back Office after enabling production mode).', 'woo-lyra-payment'), self::GATEWAY_NAME, self::BACKOFFICE_NAME),
                'custom_attributes' => array('autocomplete' => 'off')
            ),
            'ctx_mode' => array(
                'title' => __('Mode', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => self::CTX_MODE,
                'options' => array(
                    'TEST' => __('TEST', 'woo-lyra-payment'),
                    'PRODUCTION' => __('PRODUCTION', 'woo-lyra-payment')
                ),
                'description' => __('The context mode of this module.', 'woo-lyra-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'sign_algo' => array(
                'title' => __('Signature algorithm', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => self::SIGN_ALGO,
                'options' => array(
                    LyraApi::ALGO_SHA1 => 'SHA-1',
                    LyraApi::ALGO_SHA256 => 'HMAC-SHA-256'
                ),
                'description' => sprintf(__('Algorithm used to compute the payment form signature. Selected algorithm must be the same as one configured in the %s Back Office.<br /><b>The HMAC-SHA-256 algorithm should not be activated if it is not yet available in the %s Back Office, the feature will be available soon.</b>', 'woo-lyra-payment'), self::BACKOFFICE_NAME, self::BACKOFFICE_NAME),
                'class' => 'wc-enhanced-select'
            ),
            'url_check' => array(
                'title' => __('Instant Payment Notification URL', 'woo-lyra-payment'),
                'type' => 'text',
                'description' => '<span class="url">' . add_query_arg('wc-api', 'WC_Gateway_Lyra', network_home_url('/')) . '</span><br />' .
                    '<img src="' . esc_url(WC_LYRA_PLUGIN_URL . 'assets/images/warn.png') . '"><span class="desc">' . sprintf(__('URL to copy into your %s Back Office > Settings > Notification rules.', 'woo-lyra-payment'), self::BACKOFFICE_NAME) . '</span>',
                'css' => 'display: none;'
            ),
            'platform_url' => array(
                'title' => __('Payment page URL', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => self::GATEWAY_URL,
                'description' => __('Link to the payment page.', 'woo-lyra-payment'),
                'css' => 'width: 350px;'
            ),

            // Add REST API key fields.
            'rest_settings' => array(
                'title' => __('REST API keys', 'woo-lyra-payment'),
                'type' => 'title',
                'description' => sprintf(__('REST API keys are available in your %s Back Office (menu: Settings > Shops > REST API keys).<br><br>Configure this section if you are using order operations from WooCommerce Back Office, if you are using embedded payment fields/Smartform modes or if you are proposing subscription payment with WooCommerce Subscriptions.', 'woo-lyra-payment'), self::BACKOFFICE_NAME),
             ),
             'test_private_key' => array(
                 'title' => __('Test password', 'woo-lyra-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'prod_private_key' => array(
                 'title' => __('Production password', 'woo-lyra-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'rest_url' => array(
                'title' => __('API REST server URL', 'woo-lyra-payment'),
                'type' => 'text',
            ...
            'embedded_fields_keys_settings' => array(
                'title' => '',
                'type' => 'title',
                'description' => sprintf(__('Configure this section only if you are using embedded payment fields or Smartform modes.', 'woo-lyra-payment'), self::BACKOFFICE_NAME)
            ),
             'test_public_key' => array(
                 'title' => __('Public test key', 'woo-lyra-payment'),
                 'type' => 'text',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'prod_public_key' => array(
                 'title' => __('Public production key', 'woo-lyra-payment'),
                 'type' => 'text',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'test_return_key' => array(
                 'title' => __('HMAC-SHA-256 test key', 'woo-lyra-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
             'prod_return_key' => array(
                 'title' => __('HMAC-SHA-256 production key', 'woo-lyra-payment'),
                 'type' => 'password',
                 'default' => '',
                 'custom_attributes' => array('autocomplete' => 'off')
             ),
            'static_url' => array(
                'title' => __('JavaScript client URL', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => self::STATIC_URL,
                'css' => 'width: 350px;'
            ),
            'rest_check_url' => array(
                'title' => __('API REST Notification URL', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => '',
                'description' => '<span class="url">' . add_query_arg('wc-api', 'WC_Gateway_Lyra_Notify_Rest', network_home_url('/')). '</span><br />' .
                '<img src="' . esc_url(WC_LYRA_PLUGIN_URL . 'assets/images/warn.png') . '"><span class="desc">' . sprintf(__('URL to copy into your %s Back Office > Settings > Notification rules.<br>In multistore mode, notification URL is the same for all the stores.', 'woo-lyra-payment'), self::GATEWAY_NAME). '</span>',
                'css' => 'display: none;'
            ),

            // Payment page params.
            'payment_page' => array(
                'title' => __('PAYMENT PAGE', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'language' => array(
                'title' => __('Default language', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => self::LANGUAGE,
                'options' => $this->get_supported_languages(),
                'description' => __('Default language on the payment page.', 'woo-lyra-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'available_languages' => array(
                'title' => __('Available languages', 'woo-lyra-payment'),
                'type' => 'multiselect',
                'default' => array(),
                'options' => $this->get_supported_languages(),
                'description' => __('Languages available on the payment page. If you do not select any, all the supported languages will be available.', 'woo-lyra-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'capture_delay' => array(
                'title' => __('Capture delay', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => '',
                'description' => sprintf(__('The number of days before the bank capture (adjustable in your %s Back Office).', 'woo-lyra-payment'), self::BACKOFFICE_NAME)
            ),
            'validation_mode' => array(
                'title' => __('Validation mode', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => '',
                'options' => $this->get_validation_modes(true),
                'description' => sprintf(__('If manual is selected, you will have to confirm payments manually in your %s Back Office.', 'woo-lyra-payment'), self::BACKOFFICE_NAME),
                'class' => 'wc-enhanced-select'
            ),

            // Selective 3DS.
            'selective_3ds' => array(
                'title' => __('CUSTOM 3DS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            '3ds_min_amount' => array(
                'title' => __('Manage 3DS', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => '',
                'description' => __('Amount below which customer could be exempt from strong authentication. Needs subscription to «Selective 3DS1» or «Frictionless 3DS2» options. For more information, refer to the module documentation.', 'woo-lyra-payment')
            ),

            // Return to store params.
            'return_options' => array(
                'title' => __('RETURN OPTIONS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'redirect_enabled' => array(
                'title' => __('Automatic redirection', 'woo-lyra-payment'),
                'label' => __('Enable / disable', 'woo-lyra-payment'),
                'type' => 'checkbox',
                'default' => 'no',
                'description' => __('If enabled, the buyer is automatically redirected to your site at the end of the payment.', 'woo-lyra-payment')
            ),
            'redirect_success_timeout' => array(
                'title' => __('Redirection timeout on success', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => '5',
                'description' => __('Time in seconds (0-300) before the buyer is automatically redirected to your website after a successful payment.', 'woo-lyra-payment')
            ),
            'redirect_success_message' => array(
                'title' => __('Redirection message on success', 'woo-lyra-payment'),
                'type' => 'multilangtext',
                'default' => array(
                ...
                'description' => __('Message displayed on the payment page prior to redirection after a successful payment.', 'woo-lyra-payment'),
                'css' => 'width: 35em;'
            ),
            'redirect_error_timeout' => array(
                'title' => __('Redirection timeout on failure', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => '5',
                'description' => __('Time in seconds (0-300) before the buyer is automatically redirected to your website after a declined payment.', 'woo-lyra-payment')
            ),
            'redirect_error_message' => array(
                'title' => __('Redirection message on failure', 'woo-lyra-payment'),
                'type' => 'multilangtext',
                'default' => array(
                    'en_US' => 'Redirection to shop in a few seconds...',
                    'en_GB' => 'Redirection to shop in a few seconds...',
                ...
                'description' => __('Message displayed on the payment page prior to redirection after a declined payment.', 'woo-lyra-payment'),
                'css' => 'width: 35em;'
            ),
            'return_mode' => array(
                'title' => __('Return mode', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => 'POST',
                'options' => array(
                    'GET' => 'GET',
                ...
                'description' => __('Method that will be used for transmitting the payment result from the payment page to your shop.', 'woo-lyra-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'order_status_on_success' => array(
                'title' => __('Order Status', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => 'default',
                'options' => self::get_success_order_statuses(true),
                'description' => __('Defines the status of orders paid with this payment mode.', 'woo-lyra-payment'),
                'class' => 'wc-enhanced-select'
            ),
            'delete_order_on_failure' => array(
                'title' => __('Delete order on failure', 'woo-lyra-payment'),
                'label' => __('Enable / disable', 'woo-lyra-payment'),
                'type' => 'checkbox',
                'default' => 'no',
                'description' => __('If enabled, the order will be deleted on WooCommerce when the payment is declined.', 'woo-lyra-payment')
            ),

            // Additional options.
            'additional_options' => array(
                'title' => __('ADDITIONAL OPTIONS', 'woo-lyra-payment'),
                'type' => 'title',
                'description' => __('Configure this section if you are using advanced risk assessment module or if you are proposing payment means requiring cart data (Franfinance or Klarna).', 'woo-lyra-payment')
            ),
            'common_category' => array(
                'custom_attributes' => array(
                    'onchange' => 'lyraUpdateCategoryDisplay()'
                ),
                'title' => __('Category mapping', 'woo-lyra-payment'),
                'type' => 'select',
                'options' => $this->get_gateway_categories(false),
                'description' => __('Use the same category for all products.', 'woo-lyra-payment'),
                'default' => '',
                'class' => 'wc-enhanced-select'
            ),
        );
        ...
        $columns['label'] = array(
            'title' => __('Product category', 'woo-lyra-payment'),
            'width' => '154px'
        );

        $columns['category'] = array(
            'title' => sprintf(__('%s category', 'woo-lyra-payment'), self::BACKOFFICE_NAME),
            'width' => '154px'
        );
        ...
        $this->form_fields['category_mapping'] = array(
            ...
            'columns' => $columns,
            'description' => sprintf(__('Match each product category with a %s product category. <br /><b>Entries marked with * are newly added and must be configured.</b>', 'woo-lyra-payment'), self::BACKOFFICE_NAME)
        );
        ...
        $descr = sprintf(__('Define the %s information about all shipping methods.<br /><b>Method title: </b>The label of the shipping method.<br /><b>Type: </b>The delivery type of shipping method.<br /><b>Rapidity: </b>Select the delivery rapidity.<br /><b>Delay: </b>Select the delivery delay if rapidity is &laquo; Priority &raquo;.<br /><b>Entries marked with * are newly added and must be configured.</b>',
            'woo-lyra-payment'), 'Lyra Collect');

        $columns = array();
        $columns['method_title'] = array(
            'title' => __('Method title', 'woo-lyra-payment'),
            'width' => '210px'
        );

        $columns['type'] = array(
            'title' => __('Type', 'woo-lyra-payment'),
            'width' => '130px'
        );

        $columns['speed'] = array(
            'title' => __('Rapidity', 'woo-lyra-payment'),
            'width' => '75px',
        );

        $columns['delay'] = array(
            'title' => __('Delay', 'woo-lyra-payment'),
            'width' => '90px',
        );

        $this->form_fields['shipping_options'] = array(
            'title' => __('Shipping options', 'woo-lyra-payment'),
            'type' => 'shipping_table',
            'columns' => $columns,
            'description' => $descr
        );

        if (isset($lyra_plugin_features['qualif']) && $lyra_plugin_features['qualif']) {
        ...
        if (isset($lyra_plugin_features['shatwo']) && $lyra_plugin_features['shatwo']) {
    ...
    {
        $this->general_settings = get_option('woocommerce_lyra_settings', null);
        ...
        <tr valign="top">
            <th scope="row" class="titledesc">
                <contact-support
                    ...
                    cc-emails=""
                    phone-number=""
                    language="<?php echo LyraTools::get_support_component_language(); ?>"></contact-support>
            </th>
        </tr>

        <?php
        // Load css and add spinner.
        wp_register_style('lyra', WC_LYRA_PLUGIN_URL . 'assets/css/lyra.css', array(), self::PLUGIN_VERSION);
    ...
    public function generate_table_html($key, $data)
    {
        ...
        $html .= '<input id="' . $field_name . '_btn" class="' . $field_name . '_btn ' . esc_attr($data['class']) . '"' . (! empty($options) ? ' style="display: none;"' : '') . ' type="button" value="' . __('Add', 'woo-lyra-payment') . '">';
        ...
        $html .= "\n" . 'jQuery(".' . $field_name . '_btn").click(function() {
                            lyraAddOption("' . $field_name . '", ' . json_encode($record) . ');
                         })';

        if (! empty($options)) {
            // Add already inserted lines.
            foreach ($options as $code => $option) {
                $html .= "\n" . 'lyraAddOption("' . $field_name . '", ' . json_encode($option) . ', "' . $code . '");';
            }
        }
    ...
    public function generate_shipping_table_html($key, $data)
    {
        ...
        $shipping_types = array(
            'PACKAGE_DELIVERY_COMPANY' => __('Delivery company', 'woo-lyra-payment'),
            'RECLAIM_IN_SHOP' => __('Reclaim in shop', 'woo-lyra-payment'),
            'RELAY_POINT' => __('Relay point', 'woo-lyra-payment'),
            'RECLAIM_IN_STATION' => __('Reclaim in station', 'woo-lyra-payment')
        );

        $shipping_rapidities = array(
            'STANDARD' => __('Standard', 'woo-lyra-payment'),
            'EXPRESS' => __('Express', 'woo-lyra-payment'),
            'PRIORITY' => __('Priority', 'woo-lyra-payment')
        );

        $shipping_delays = array(
            'INFERIOR_EQUALS' => __('<= 1 hour', 'woo-lyra-payment'),
            'SUPERIOR' => __('> 1 hour', 'woo-lyra-payment'),
            'IMMEDIATE' => __('Immediate', 'woo-lyra-payment'),
            'ALWAYS' =>  __('24/7', 'woo-lyra-payment'),
        );
        ...
        foreach ($options as $code => $option) {
            ...
            $html .= '<td style="padding: 5px;"><select class="lyra_list_speed" id="' . $field_name . '_' . $code . '_speed"
                      name="' . $field_name . '[' . $code . '][speed]"
                      value="' . (isset($option['speed']) ? $option['speed'] : '') . '"
                      onchange="javascript:lyraUpdateShippingOptionsDisplay(\'' . $field_name . '_' . $code . '_speed\')">';
    ...
    public function validate_ctx_mode_field($key, $value = null)
    {
        ...
        if (! $new_value && $lyra_plugin_features['qualif']) {
    ...
    {
        $currency = LyraApi::findCurrencyByAlphaCode(get_woocommerce_currency());
        if ($currency == null) {
            return false;
        }
     ...
     **/
    public function lyra_notify_response()
    {
        global $woocommerce;
        ...
        if ($session_id = $lyra_response->getExtInfo('session_id')) {
            set_transient('lyra_session_id', $session_id);
        }

        $from_server = $lyra_response->get('hash') != null;
        ...
        if (! $lyra_response->isAuthentified()) {
            ...
            if ($from_server) {
                ...
                die($lyra_response->getOutputForGateway('auth_fail'));
            } else {
                // Fatal error, empty cart.
                $woocommerce->cart->empty_cart();
                $this->add_notice(__('An error has occurred in the payment process.', 'woo-lyra-payment'), 'error');

                $this->log('RETURN URL PROCESS END');

                $cart_url = function_exists('wc_get_cart_url') ? wc_get_cart_url() : $woocommerce->cart->get_cart_url();
                $iframe = $lyra_response->get('action_mode') === 'IFRAME';
                $this->lyra_redirect($cart_url, $iframe);
            }
        } else {
            header('HTTP/1.1 200 OK');

            $this->lyra_manage_notify_response($lyra_response);
        }
    }
     ...
     **/
    public function lyra_manage_notify_response($lyra_response, $from_server_rest = false, $update_on_failure = true, $server_rest_abandoned_payment = false)
    {
        global $woocommerce, $lyra_plugin_features;
        ...
        $from_server = $lyra_response->get('hash') != null || $from_server_rest;
        $iframe = $lyra_response->get('action_mode') == 'IFRAME';
        ...
        $key = $lyra_response->getExtInfo('order_key');

        $order = wc_get_order($order_id);

        if ($server_rest_abandoned_payment && LyraTools::is_embedded_payment(false) && empty($lyra_response->getTransStatus())) {
        ...
        if (! $order && ($this->get_general_option('delete_order_on_failure') == "yes")
            && ! self::is_successful_action($lyra_response)) {
            $this->log("Order #$order_id was deleted on payment failure. Please, try to re-order.");

            if ($from_server) {
                $this->log('IPN URL PROCESS END');
                die($lyra_response->getOutputForGateway('payment_ko'));
            } else {
                $this->log('RETURN URL PROCESS END');

                if (! $lyra_response->isCancelledPayment()) {
                    $this->add_notice(__('Your payment was not accepted. Please, try to re-order.', 'woo-lyra-payment'), 'error');
                    $this->lyra_redirect($cart_url, $iframe);
                } else {
                    $this->lyra_redirect($checkout_url, $iframe);
                }
            }
        }
        ...
        if (! $order || ! self::get_order_property($order, 'id') || ($key && (self::get_order_property($order, 'order_key') !== $key))) {
            ...
            if ($from_server) {
                ...
                header(self::HEADER_ERROR_500, true, 500);
                die($lyra_response->getOutputForGateway('order_not_found'));
            } else {
                // Fatal error, empty cart.
                $woocommerce->cart->empty_cart();
                $this->add_notice(__('An error has occurred in the payment process.', 'woo-lyra-payment'), 'error');
        ...
        if (! $from_server && $this->testmode && $lyra_plugin_features['prodfaq']) {
            $msg = __('<p><u>GOING INTO PRODUCTION</u></p>You want to know how to put your shop into production mode, please read chapters « Proceeding to test phase » and « Shifting the shop to production mode » in the documentation of the module.', 'woo-lyra-payment');
        ...
        if (LyraRestTools::checkResponse($_POST)) {
            $lyra_std = new WC_Gateway_LyraStd();
            if (method_exists($order, 'set_payment_method')) {
                $order->set_payment_method($lyra_std);
            } else {
                $order->payment_method = $lyra_std->id;
                $order->payment_method_title = $lyra_std->get_title();
            }
        ...
        if ($this->is_new_order($order, $lyra_response->get('trans_id'), $lyra_response->get('sequence_number'))) {
            // Clear transients.
            delete_transient('lyra_token_data_' . $lyra_response->getExtInfo('session_id'));
            delete_transient('lyra_token_data_identifier_' . $lyra_response->getExtInfo('session_id'));
            delete_transient('lyra_token_' . $lyra_response->getExtInfo('session_id'));
            delete_transient('lyra_token_identifier_' . $lyra_response->getExtInfo('session_id'));

            // Add order note.
            self::lyra_add_order_note($lyra_response, $order);

            self::lyra_update_order_meta($lyra_response, $order);

            if (self::is_successful_action($lyra_response)) {
                ...
                $this->lyra_save_identifier($order, $lyra_response);

                if ($subscriptions_handler) {
                    // Try to save subscritption info if any.
                    $this->lyra_save_recurrence($order, $lyra_response);

                    $subscriptions_handler->process_subscription($order, $lyra_response);
                }
                ...
                if ($from_server) {
                    ...
                    die($lyra_response->getOutputForGateway('payment_ok'));
                } else {
                    $this->log("Warning! IPN URL call has not worked. Payment completed by return URL call for order #$order_id.");

                    if ($this->testmode) {
                        $ipn_url_warn = sprintf(__('The automatic validation has not worked. Have you correctly set up the notification URL in the %s Back Office?', 'woo-lyra-payment'), self::BACKOFFICE_NAME);
                        $ipn_url_warn .= '<br />';
                        $ipn_url_warn .= __('For understanding the problem, please read the documentation of the module : <br />&nbsp;&nbsp;&nbsp;- Chapter &laquo; To read carefully before going further &raquo;<br />&nbsp;&nbsp;&nbsp;- Chapter &laquo; Notification URL settings &raquo;', 'woo-lyra-payment');

                        $this->add_notice($ipn_url_warn, 'error');
                    }

                    $this->log('RETURN URL PROCESS END');
                    $this->lyra_redirect($this->get_return_url($order), $iframe);
                }
            } else {
                $this->log("Payment failed or cancelled for order #$order_id. {$lyra_response->getLogMessage()}");
                if ($from_server) {
                ...
                if ($from_server) {
                    $this->log('IPN URL PROCESS END');
                    die($lyra_response->getOutputForGateway($msg));
                } else {
                    $this->log('RETURN URL PROCESS END');
            ...
            $wcs_scheduled = ($method === 'lyrawcssubscription');

            // Case of new recurrence on an active subscription with our method.
            if ($lyra_response->get('recurrence_number') && $subscriptions_handler) {
                ...
                $subscriptions_handler->process_subscription_renewal($order, $lyra_response);

                if (self::is_successful_action($lyra_response)) {
                    $this->log("Payment recurrence processed successfully by IPN URL call for order #$order_id.");
                    echo($lyra_response->getOutputForGateway('payment_ok'));
                } else {
                    echo($lyra_response->getOutputForGateway('payment_ko'));
                }

                $this->log('IPN URL PROCESS END');
                die();
            } elseif (($lyra_response->get('identifier_status') === 'UPDATED')
                || (($lyra_response->get('identifier_status') === 'CREATED') && $wcs_scheduled)
                    /* Case of method change from WooCommerce or subscription creation on gateway Back Office. */
                    || ((($lyra_response->get('recurrence_status') === 'CREATED') || ($lyra_response->get('recurrence_number') === '1'))
                        && ($subscriptions_handler = self::subscriptions_handler('lyrasubscription')))) {
                ...
                if ($subsc_id = $lyra_response->getExtInfo('subsc_id')) {
                    $subsc_redirect_url = isset($subscriptions_handler) ? $subscriptions_handler->get_view_order_url($subsc_id) : LyraTools::get_view_order_url($subsc_id);
                }
                ...
                if (self::is_successful_action($lyra_response)) {
                    ...
                    $force_update = $lyra_response->get('recurrence_number') === '1';

                    if ($force_update && ($method !== 'lyrasubscription')) {
                        // Update payment method in order.
                        $lyra_subscription = new WC_Gateway_LyraSubscription();
                        if (method_exists($order, 'set_payment_method')) {
                            $order->set_payment_method($lyra_subscription);
                        } else {
                            $order->payment_method = $lyra_subscription->id;
                            $order->payment_method_title = $lyra_subscription->get_title();
                        }
                    ...
                    // Try to save identifier if any.
                    $this->lyra_save_identifier($order, $lyra_response, $force_update);

                    // Try to save subscription info if any.
                    $this->lyra_save_recurrence($order, $lyra_response, $force_update);
                    ...
                    if ($from_server) {
                        $this->log('IPN URL PROCESS END');
                        die($lyra_response->getOutputForGateway('payment_ok_already_done'));
                    } else {
                        $this->add_notice(__('Payment method updated.', 'woocommerce-subscriptions'));

                        $this->log('RETURN URL PROCESS END');
                        $this->lyra_redirect($subsc_redirect_url ? $subsc_redirect_url : $this->get_return_url($order), $iframe);
                    }
                } else {
                    if ($from_server) {
                        $this->log('IPN URL PROCESS END');
                        ...
                        if (! $lyra_response->isCancelledPayment()) {
                            ...
                            $this->lyra_redirect($subsc_redirect_url ? $subsc_redirect_url : $cart_url, $iframe);
                        } else {
                            $this->lyra_redirect($subsc_redirect_url ? $subsc_redirect_url : $checkout_url, $iframe);
                        }
                    }
                }
            }
            ...
            if ($from_server && ($order_status === 'on-hold')) {
                switch (true) {
                    case $lyra_response->isCancelledPayment():
                        $this->log("Order #$order_id is in a pending status and payment is cancelled. It may be a payment expiration. Do nothing.");
                        echo($lyra_response->getOutputForGateway('payment_ko_already_done'));
                        break;
                    case $lyra_response->isPendingPayment():
                        $this->log("Order #$order_id is in a pending status and stays in the same status. Do nothing.");
                        echo($lyra_response->getOutputForGateway('payment_ok_already_done'));
                        break;
                    case self::is_successful_action($lyra_response):
                ...
                $this->log('IPN URL PROCESS END');
                die();
            } elseif (self::is_successful_action($lyra_response) && key_exists($order_status, self::get_success_order_statuses(false, $subscriptions_handler))) {
                $status = $lyra_response->isPendingPayment() ? 'pending' : 'successful';
            ...
            } elseif (! self::is_successful_action($lyra_response) && ($order_status === 'failed' || $order_status === 'cancelled')) {
                $this->log("Payment failed confirmed for order #$order_id.");

                // Order failure registered and payment error received.
                if ($from_server) {
                    $this->log('IPN URL PROCESS END');
                    die($lyra_response->getOutputForGateway('payment_ko_already_done'));
                } else {
                    $this->log('RETURN URL PROCESS END');
                ...
                $this->log("Error! Invalid payment result received for already saved order #$order_id. Payment result : {$lyra_response->getTransStatus()}, Order status : {$order_status}.");
                ...
                if ($from_server) {
                    ...
                    header(self::HEADER_ERROR_500, true, 500);
                    die($lyra_response->getOutputForGateway('payment_ko_on_order_ok'));
                } else {
                    // Fatal error, empty cart.
                    $woocommerce->cart->empty_cart();
                    $this->add_notice(__('An error has occurred in the payment process.', 'woo-lyra-payment'), 'error');
    ...
    public function lyra_complete_order_status($status, $order_id)
    {
        $order = new WC_Order((int)$order_id);

        if ((strpos(self::get_order_property($order, 'payment_method'), 'lyra') === 0)
    ...
    {
        if ($order->has_status(apply_filters('woocommerce_lyra_valid_order_statuses', array('pending', 'checkout-draft'), $order))) {
            return true;
        }
        ...
        if (function_exists('wcs_get_subscription')) {
            ...
            if ($subscription && strpos(self::get_order_property($order, 'payment_method'), 'lyra') === false) {
                return true;
            }
        }

        if (LyraTools::is_hpos_enabled()) {
    ...
    protected function clear_notices()
    {
        ...
        delete_transient('lyra_notices_' . wp_get_session_token());
    ...
    protected function add_notice($msg, $type = 'success')
    {
        ...
        if (function_exists('wc_add_notice')) {
            ...
            if (LyraTools::has_checkout_block() && ($type == 'error')) {
                $type = 'notice';
            }
    ...
    public function lyra_add_order_email_payment_result($order, $sent_to_admin, $plain_text = false)
    {
        if (strpos(self::get_order_property($order, 'payment_method'), 'lyra') !== 0) {
            return;
        }
        ...
        foreach ($notes as $note) {
            if (strpos($note, $trans_id) !== false) {
                $lyra_order_note = $note;
                break;
            }
        }

        if (isset($lyra_order_note)) {
            if ($plain_text) {
                echo strtoupper(__('Payment', 'woo-lyra-payment')) . "\n\n" . $lyra_order_note . "\n\n";
            } else {
                echo '<h2>' . __('Payment', 'woo-lyra-payment') . '</h2><p>' . str_replace("\n", '<br >', $lyra_order_note) . '</p>';
            }
        }
    }
    ...
    protected function lyra_redirect($url, $iframe = false)
    {
        // Save WC Notices to restore them on return page.
        $wc_notices = WC()->session->get('wc_notices', array());
        if (($session_id = get_transient('lyra_session_id')) && ! empty($wc_notices)) {
            set_transient('lyra_notices_' . $session_id, json_encode($wc_notices));
            delete_transient('lyra_session_id');
        }
        ...
        if (! $iframe) {
            ...
            echo '<div style="text-align: center;">
                      <img src="' . esc_url(WC_LYRA_PLUGIN_URL . 'assets/images/loading_big.gif') . '">
                  </div>';
    ...
    public static function lyra_add_order_note($lyra_response, $order)
    {
        $note = $lyra_response->getCompleteMessage("\n");

        if ($lyra_response->get('brand_management')) {
            $brand_info = json_decode($lyra_response->get('brand_management'));
            $msg_brand_choice = "\n";

            if (isset($brand_info->userChoice) && $brand_info->userChoice) {
                $msg_brand_choice .= __('Card brand chosen by buyer.', 'woo-lyra-payment');
            } else {
                $msg_brand_choice .= __('Default card brand used.', 'woo-lyra-payment');
            }
        ...
        // 3DS extra message.
        $note = __('3DS authentication: ', 'woo-lyra-payment');
        if ($status = $lyra_response->get('threeds_status')) {
            $note .= self::get_threeds_status($status);

            if ($threeds_cavv = $lyra_response->get('threeds_cavv')) {
                $note .= "\n";
                $note .= __('3DS certificate: ', 'woo-lyra-payment') . $threeds_cavv;
            }

            if ($threeds_auth_type = $lyra_response->get('threeds_auth_type')) {
                $note .= "\n";
                $note .= __('Authentication type: ', 'woo-lyra-payment') . $threeds_auth_type;
            }
        } else {
            $note .= 'UNAVAILABLE';
        }
        ...
        $note = '';
        if (! $lyra_response->isCancelledPayment()) {
            $note .= sprintf(__('Transaction ID: %s.', 'woo-lyra-payment'), $lyra_response->get('trans_id'));

            if ($lyra_response->get('trans_uuid')) {
                $note .= "\n";
                $note .= sprintf(__('Transaction UUID: %s.', 'woo-lyra-payment'), $lyra_response->get('trans_uuid'));
            }
        }

        if ($lyra_response->getTransStatus()) {
            $note .= "\n";
            $note .= sprintf(__('Transaction status: %s.', 'woo-lyra-payment'), $lyra_response->getTransStatus());
        }

        $order->add_order_note($note);
    }

    public static function lyra_update_order_meta($lyra_response, $order)
    {
        if (LyraTools::is_hpos_enabled()) {
            ...
            $order->update_meta_data('Transaction ID', $lyra_response->get('trans_id'));
            $order->update_meta_data('Card number', $lyra_response->get('card_number'));
            $order->update_meta_data('Means of payment', $lyra_response->get('card_brand'));
            $order->update_meta_data('Sequence number', $lyra_response->get('sequence_number'));

            // Store authorized amount.
            if ($authorized_amount = $lyra_response->get('authorized_amount')) {
                $order->delete_meta_data('Authorized amount');
                $order->update_meta_data('Authorized amount', self::display_amount($authorized_amount, $lyra_response->get('currency')));
            }

            // Store installments number/config.
            if (($installments_number = $lyra_response->get('payment_option_code')) && is_numeric($installments_number)) {
                $order->delete_meta_data('Installments number');
                $order->update_meta_data('Installments number', $installments_number);
            }

            $expiry = '';
            if ($lyra_response->get('expiry_month') && $lyra_response->get('expiry_year')) {
                $expiry = str_pad($lyra_response->get('expiry_month'), 2, '0', STR_PAD_LEFT) . '/' . $lyra_response->get('expiry_year');
            }

            $order->update_meta_data('Card expiry', $expiry);
            $order->save();
        } else {
            $order_id = (int) $lyra_response->get('order_id');
            ...
            // Store transaction details.
            update_post_meta($order_id, 'Transaction ID', $lyra_response->get('trans_id'));
            update_post_meta($order_id, 'Card number', $lyra_response->get('card_number'));
            update_post_meta($order_id, 'Means of payment', $lyra_response->get('card_brand'));
            update_post_meta($order_id, 'Sequence number', $lyra_response->get('sequence_number'));

            // Store authorized amount.
            if ($authorized_amount = $lyra_response->get('authorized_amount')) {
                delete_post_meta($order_id, 'Authorized amount');
                update_post_meta($order_id, 'Authorized amount', self::display_amount($authorized_amount, $lyra_response->get('currency')));
            }
    ...
    public static function restore_wc_notices() {
        ...
        if (WC()->session && empty(WC()->session->get('wc_notices', array())) && get_transient('lyra_notices_' . wp_get_session_token())) {
            wc_set_notices(json_decode(get_transient('lyra_notices_' . wp_get_session_token()), true));
            delete_transient('lyra_notices_' . wp_get_session_token());
        }
    }

    public function lyra_notices($template_name, $template_path, $located, $args = array())
    {
        global $woocommerce;
    ...
    public function lyra_thankyou($order_id)
    {
        $order = wc_get_order($order_id);
    ...
    private static function get_success_order_statuses($default = false, $subscriptions_handler = false)
    {
        ...
        if (function_exists('wc_get_order_statuses')) { // From WooCommerce 2.2.0.
            ...
            $statuses = array(
                'on-hold' => __('On Hold', 'woo-lyra-payment'),
                'processing' => __('Processing', 'woo-lyra-payment'),
                'completed' => __('Complete', 'woo-lyra-payment')
            );
        }

        if ($default) {
            $statuses = array('default' => __('Default', 'woo-lyra-payment')) + $statuses;
        }
    ...
    private function lyra_save_identifier($order, $lyra_response, $force_update = false)
    {
        ...
        if ($lyra_response->get('identifier') && ($force_update || (
            $lyra_response->get('identifier_status') == 'CREATED' /* page_action is REGISTER_PAY or ASK_REGISTER_PAY */ ||
            $lyra_response->get('identifier_status') == 'UPDATED' /* page_action is REGISTER_UPDATE_PAY */
        ))) {
            ...
            $identifier_info = array(
                'identifier' => $lyra_response->get('identifier'),
                'active' => true
            );
            ...
            $subsc_id = $lyra_response->getExtInfo('subsc_id') ? $lyra_response->getExtInfo('subsc_id') : $lyra_response->get('order_id');
            $this->log("Saving identifier for order #$subsc_id in meta data.");

            if (LyraTools::is_hpos_enabled()) {
                $subscription = wc_get_order($subsc_id);
                $subscription->update_meta_data('lyra_token', $lyra_response->get('identifier'));
                $subscription->save();
            } else {
                update_post_meta($subsc_id, 'lyra_token', $lyra_response->get('identifier'));
            }
            ...
            if (function_exists('wcs_get_subscriptions_for_order')) {
                ...
                foreach (wcs_get_subscriptions_for_order($order) as $subscription) {
                    if (LyraTools::is_hpos_enabled()) {
                        ...
                        update_post_meta($subscription->get_id(), 'lyra_token', $lyra_response->get('identifier'));
                    }
                }
            }

            // Mask all card digits unless the last 4 ones.
            $number = $lyra_response->get('card_number');
            $masked = '';
            ...
            if (preg_match('#^([A-Z]{2}[0-9]{2}[A-Z0-9]{10,30})(_[A-Z0-9]{8,11})?$#i', $number, $matches)) {
                ...
                $masked = $lyra_response->get('card_brand') . '|' .  str_repeat('X', strlen($number) - 4) . substr($number, - 4);

                if ($lyra_response->get('expiry_month') && $lyra_response->get('expiry_year')) {
                    // Format card expiration data.
                    $masked .= ' (';
                    $masked .= str_pad($lyra_response->get('expiry_month'), 2, '0', STR_PAD_LEFT);
                    $masked .= '/';
                    $masked .= $lyra_response->get('expiry_year');
                    $masked .= ')';
                }
            }
    ...
    private function lyra_save_recurrence($order, $lyra_response, $force_update = false)
    {
        $order_id = (int) self::get_order_property($order, 'id');

        if ($lyra_response->get('subscription') && ($force_update || $lyra_response->get('recurrence_status') === 'CREATED')) {
            $this->log("Subscription for order #{$order_id} successfully created on payment gateway. Let's save subscription information.");

            $currency_code = $lyra_response->get('sub_currency');
            ...
            if (LyraTools::is_hpos_enabled()) {
                ...
                // Store subscription details.
                $order->update_meta_data('Subscription ID', $lyra_response->get('subscription'));
                $order->update_meta_data('Subscription amount', self::display_amount($lyra_response->get('sub_amount'), $currency_code));
                $order->update_meta_data('Effect date', preg_replace('#^(\d{4})(\d{2})(\d{2})$#', '\1-\2-\3', $lyra_response->get('sub_effect_date')));

                if ($lyra_response->get('sub_init_amount')) {
                    $order->update_meta_data('Initial amount', self::display_amount($lyra_response->get('sub_init_amount'), $currency_code));
                    $order->update_meta_data('Initial amount count', $lyra_response->get('sub_init_amount_number'));
                }
                ...
                // Store subscription details.
                update_post_meta($order_id, 'Subscription ID', $lyra_response->get('subscription'));
                update_post_meta($order_id, 'Subscription amount', self::display_amount($lyra_response->get('sub_amount'), $currency_code));
                update_post_meta($order_id, 'Effect date', preg_replace('#^(\d{4})(\d{2})(\d{2})$#', '\1-\2-\3', $lyra_response->get('sub_effect_date')));

                if ($lyra_response->get('sub_init_amount')) {
                    update_post_meta($order_id, 'Initial amount', self::display_amount($lyra_response->get('sub_init_amount'), $currency_code));
                    update_post_meta($order_id, 'Initial amount count', $lyra_response->get('sub_init_amount_number'));
                }
            }
    ...
    public static function display_amount($amount_in_cents, $currency_code)
    {
        ...
        $currency = LyraApi::findCurrencyByNumCode($currency_code);
    ...
    {
        if ($method !== 'lyrasubscription') {
            return null;
        }

        $settings = get_option('woocommerce_lyrasubscription_settings', null);

        $handler = is_array($settings) && isset($settings['subscriptions']) ? $settings['subscriptions'] : null;
        return Lyra_Subscriptions_Loader::getInstance($handler);
    }

    public function lyra_delete_saved_card()
    {
        global $woocommerce;
        ...
        if (is_user_logged_in() && isset($_POST['id']) && ! empty($_POST['id'])) {
            ...
            try {
                ...
                if ($key) {
                    ...
                    LyraRestTools::checkResult($result);
                ...
                if (in_array($e->getCode(), $invalid_ident_codes)) {
                    ...
                    $this->add_notice(__('The stored means of payment could not be deleted.', 'woo-lyra-payment'), 'error');
                    return false;
                }
            }
        }
    ...
    function lyra_delete_order_with_status_failed($order_id)
    {
        // Get an instance of the order object.
        $order = wc_get_order($order_id);

        if (($this->get_general_option('delete_order_on_failure') == 'yes')
            && (strpos(self::get_order_property($order, 'payment_method'), 'lyra') === 0)
    ...
    private function delete_identifier_attributes($cust_id, $id)
    {
        ...
        $this->add_notice(__('The stored means of payment was successfully deleted.', 'woo-lyra-payment'));
    }
    ...
    protected function check_identifier($cust_id, $id, $identifier_to_check = null)
    {
        ...
        try {
            ...
            $cancellation_date = LyraRestTools::getProperty($result['answer'], 'cancellationDate');
