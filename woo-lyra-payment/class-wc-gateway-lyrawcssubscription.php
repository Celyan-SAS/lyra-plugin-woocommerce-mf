...
class WC_Gateway_LyraWcsSubscription extends WC_Gateway_LyraStd
{
    ...
    {
        $this->id = 'lyrawcssubscription';
        $this->icon = apply_filters('woocommerce_' . $this->id . '_icon', WC_LYRA_PLUGIN_URL . 'assets/images/lyra.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Subscription payment with WooCommerce Subscriptions', 'woo-lyra-payment');
        $this->method_description = __('Subscriptions managed by WooCommerce Subscriptions', 'woo-lyra-payment')
            . ' <b>(' . __('Recommended', 'woo-lyra-payment') . ')</b>';
        ...
        $this->subscriptions_handler = Lyra_Subscriptions_Loader::getInstance(self::SUBSCRIPTIONS_HANDLER);
    ...
    protected function lyra_fill_request($order)
    {
        ...
        if ($saved_identifier && $is_identifier_active) {
            ...
            $action = ($this->lyra_request->get('amount') == 0) ? 'REGISTER_UPDATE' : 'REGISTER_UPDATE_PAY';
        } else {
            $action = ($this->lyra_request->get('amount') == 0) ? 'REGISTER' : 'REGISTER_PAY';
        }

        $this->lyra_request->set('page_action', $action);

        // Payment schedule is managed by WooCommerce Subscriptions.
        $this->lyra_request->addExtInfo('wcs_scheduled', true);
    }
    ...
    {
        if (LyraTools::is_plugin_not_active('woocommerce-subscriptions/woocommerce-subscriptions.php') === 'true') {
    ...
    public function process_subscription_payment($renewal_total, WC_Order $renewal_order)
    {
        ...
        if (! $key) {
            $error_msg = __('Private key is not configured. Subscription renewal cannot be processed.', 'woo-lyra-payment');
        ...
        if (! $saved_identifier) {
            ...
            $error_msg = __('Customer has no valid identifier. Subscription renewal cannot be processed.', 'woo-lyra-payment');
            $this->set_error($renewal_order, $subscription, $error_msg);
        ...
        if (! $payment_result) {
            $error_msg = sprintf(__('An error has occurred during the renewal of the subscription. Please consult the %s logs for more details.', 'woo-lyra-payment'), 'Lyra Collect');
            $this->set_error($renewal_order, $subscription, $error_msg);

            return;
        }

        $data = LyraRestTools::convertRestResult($payment_result);
    ...
    public function update_renewal_order($renewal_order, $response)
    {
        ...
        try {
            $order_note = sprintf(_x('Processed renewal payment for order #%s.', 'used in order note', 'woo-lyra-payment'), $renewal_order_id);
            $subscription->add_order_note($order_note);

            WC_Gateway_Lyra::lyra_add_order_note($response, $renewal_order);

            if (WC_Gateway_Lyra::is_successful_action($response)) {
                // Payment completed.
            ...
            $order_note = sprintf(__("Error while processing order: %s", 'woo-lyra-payment'), $e->getMessage());
    ...
    protected function silent_payment($renewal_total, WC_Order $renewal_order, $saved_identifier)
    {
        ...
        $currency = LyraApi::findCurrencyByAlphaCode($renewal_order->get_currency());

        $subscriptions = wcs_get_subscriptions_for_order($renewal_order, array('order_type' => array('renewal')));
        $subscription = reset($subscriptions); // Get first subscription.

        $this->lyra_fill_request($renewal_order);
        ...
        $validationMode = $this->get_escaped_var($this->lyra_request, 'validation_mode');
    ...
    {
        $saved_meta = $subscription->get_meta('lyra_token');
        ...
        $payment_meta[$this->id] = array(
            'post_meta' => array(
                'lyra_token' => array(
                    'value' => $saved_meta,
                    'label' => sprintf(__('%s token', 'woo-lyra-payment'), self::GATEWAY_NAME)
                )
            )
        );
    ...
    private function get_identifier($cust_id, $subscription)
    {
        $saved_identifier = $this->get_cust_identifier($cust_id);
        if (! $saved_identifier) {
            $saved_identifier = $subscription->get_meta('lyra_token');
        }
    ...
    protected function set_error($order, $subscription, $error_msg)
    {
        ...
        if (is_admin()) { // Show error message only if it's made on backend.
            set_transient('lyra_renewal_error_msg', $error_msg);
        }
    }
    ...
    public function process_admin_options() {
        ...
        $subscription_settings = get_option('woocommerce_lyrasubscription_settings', null);
        ...
        WC_Admin_Settings::add_error(sprintf(__('This method cannot be enabled. You have to disable "%s" method first.', 'woo-lyra-payment'),
            self::GATEWAY_NAME . ' - ' . __('Subscription payment', 'woo-lyra-payment')));
