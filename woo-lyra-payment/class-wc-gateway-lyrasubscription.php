...
class WC_Gateway_LyraSubscription extends WC_Gateway_LyraStd
{
    ...
    {
        $this->id = 'lyrasubscription';
        $this->icon = apply_filters('woocommerce_lyrasubscription_icon', WC_LYRA_PLUGIN_URL . 'assets/images/lyra.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Subscription payment', 'woo-lyra-payment');
        $this->method_description = sprintf(__('Subscriptions managed by %s gateway', 'woo-lyra-payment'), self::GATEWAY_NAME)
            . ' <b>('. __('Deprecated', 'woo-lyra-payment') . ')</b>';
        ...
        $this->subscriptions_handler = Lyra_Subscriptions_Loader::getInstance($handler);
        ...
        add_filter('woocommerce_order_needs_payment', array($this, 'lyra_order_needs_payment'), 10, 2);

        add_action('manage_shop_subscription_posts_custom_column', array($this, 'lyra_display_subscription_error_msg'), 2);
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables payment by Subscription.', 'woo-lyra-payment');

        // Add subscription payment fields.
        $this->form_fields['subscriptions'] = array(
            'custom_attributes' => array(
                'onchange' => 'lyraShowSubscriptionsWarningMessage()'
            ),
            'title' => __('Subscriptions management', 'woo-lyra-payment'),
            'type' => 'select',
            'default' => 'wc-subscriptions',
            'options' => array(
                'wc-subscriptions' => 'WooCommerce Subscriptions',
                'subscriptio' => 'Subscriptio 2.x',
                'custom' => __('Custom', 'woo-lyra-payment')
            ),
            'description' => __('If you buy subscriptions on your site, choose the solution you use to manage them. If you choose "Custom", your developper may develop a subscriptions adapter for our plugin.', 'woo-lyra-payment'),
    ...
    public function payment_fields()
    {
        ...
        if ($this->subscriptions_handler && $this->subscriptions_handler->is_subscription_update()) {
            ...
            echo '<input type="hidden" id="lyrasubscription_old_pm" name="lyrasubscription_old_pm" value="' . $method . '">';
        }
    }
        ...
        return '<div id="' . $this->id . '_payment_by_token_description">
                  <ul>
                      <li style="list-style-type: none;">
                          <span>' .
                              sprintf(__('You will pay with your stored means of payment %s', 'woo-lyra-payment'), $saved_subsc_masked_pan)
    ...
    protected function lyra_fill_request($order)
    {
        ...
        if (is_array($info) && ! empty($info)) {
            ...
            $this->lyra_request->set('sub_amount', $currency->convertAmountToInteger($info['amount']));
            $this->lyra_request->set('sub_currency', $currency->getNum()); // Same as general order currency.

            $this->lyra_request->set('sub_desc', $this->get_rrule($info));

            $this->lyra_request->set('sub_effect_date', $info['effect_date']);

            // Initial amount.
            if (isset($info['init_amount']) && $info['init_amount'] && isset($info['init_number']) && $info['init_number']) {
                $this->lyra_request->set('sub_init_amount', $currency->convertAmountToInteger($info['init_amount']));
                $this->lyra_request->set('sub_init_amount_number', $info['init_number']);
            }

            $order_amount = $order->get_total();
            if ($order_amount > 0) {
                $this->lyra_request->set('page_action', 'REGISTER_PAY_SUBSCRIBE');
                $this->lyra_request->set('identifier', null);
            } else {
                // Only subscriptions.
                if ($saved_identifier && $is_identifier_active) {
                    $this->lyra_request->set('identifier', $saved_identifier);
                    $this->lyra_request->set('page_action', 'SUBSCRIBE');
                } else {
                    $this->lyra_request->set('page_action', 'REGISTER_SUBSCRIBE');
                }
            }
        } elseif ($saved_identifier) {
            // Called from change payment action.
            $this->lyra_request->set('amount', 0);
            $this->lyra_request->set('identifier', $saved_identifier);
            $this->lyra_request->set('page_action', 'REGISTER_UPDATE');
        }
        ...
        if ($this->subscriptions_handler && ($parent_order = $this->subscriptions_handler->get_parent_order($order_id))) {
            $this->lyra_request->set('order_id', self::get_order_property($parent_order, 'id'));
            $this->lyra_request->addExtInfo('order_key', self::get_order_property($parent_order, 'order_key'));
            $this->lyra_request->addExtInfo('subsc_id', $order_id);
        }
    }
    ...
    public function cancel_online_subscription($subscription_id, $order_id)
    {
        ...
        if (! $key) {
            if (is_admin()) { // Show error message only if it's made on backend.
                set_transient('lyra_cancelled_subscription_error_msg', sprintf(__('Subscription is cancelled only in WooCommerce. Please, consider cancelling the subscription in %s Back Office.', 'woo-lyra-payment'), 'Lyra Expert'));
            } else {
                wc_add_notice(__('An error occurred during the cancellation of the subscription. Please contact customer support.', 'woo-lyra-payment'), 'notice');
            }
        ...
        $cust_id = self::get_order_property($order, 'user_id');
        $saved_identifier = $this->get_cust_identifier($cust_id);
        $subscriptionId = (LyraTools::is_hpos_enabled()) ? $order->get_meta('Subscription ID') : get_post_meta($order_id, 'Subscription ID', true);
        ...
        try {
            ...
            if (is_admin()) { // Show error message only if it's made on backend.
                set_transient('lyra_cancelled_subscription_error_msg', sprintf(__('An error has occurred during the cancellation or update of the subscription. Please consult the %s logs for more details.', 'woo-lyra-payment'), 'Lyra Collect'));
            } else {
                wc_add_notice(__('An error occurred during the cancellation of the subscription. Please contact customer support.', 'woo-lyra-payment'), 'notice');
            }
    ...
    private function cancel_transaction($transaction)
    {
        ...
        try {
            ...
            LyraRestTools::checkResult($result, 'CANCELLED');
        } catch (Exception $e) {
            if (is_admin()) { // Show error message only if it's made on backend.
                set_transient('lyra_cancelled_subscription_error_msg', sprintf(__('An error has occurred during the cancellation or update of the subscription. Please consult the %s logs for more details.', 'woo-lyra-payment'), 'Lyra Collect'));
            }  else {
    ...
    public function update_online_subscription($subscription_id, $order_id)
    {
        $key = $this->testmode ? $this->get_general_option('test_private_key') : $this->get_general_option('prod_private_key');
        if (! $key) {
            set_transient('lyra_update_subscription_error_msg', sprintf(__('Subscription is updated only in WooCommerce. Please, consider making necessary changes in %s Back Office.', 'woo-lyra-payment'), 'Lyra Expert'));
        ...
        $info = $this->subscriptions_handler ? $this->subscriptions_handler->subscription_info($order) : null;
        if (! is_array($info) || empty($info)) {
            set_transient('lyra_update_subscription_error_msg', sprintf(__('An error has occurred during the cancellation or update of the subscription. Please consult the %s logs for more details.', 'woo-lyra-payment'), 'Lyra Collect'));
        ...
        $subscriptionId = (LyraTools::is_hpos_enabled()) ? $order->get_meta('Subscription ID', true) : get_post_meta($order_id, 'Subscription ID', true);
    ...
    private function get_order_details($order_id)
    {
        ...
        try {
            ...
            LyraRestTools::checkResult($get_order_response);
    ...
    public function lyra_order_needs_payment($is_active, $order)
    {
        global $woocommerce;
    ...
    public function lyra_display_subscription_error_msg($column)
    {
        $lyra_cancelled_subscription_error_msg = get_transient('lyra_cancelled_subscription_error_msg');
        if ($lyra_cancelled_subscription_error_msg) {
            delete_transient('lyra_cancelled_subscription_error_msg');
            ?>
            <script type="text/javascript">
                jQuery(document).ready(function() {
                    if (! jQuery('#lyra_cancelled_subscription_error').length) {
                        jQuery('a.page-title-action').after('<div id="lyra_cancelled_subscription_error" class="error notice is-dismissible"><p><?php echo addslashes($lyra_cancelled_subscription_error_msg); ?></p><button type="button" class="notice-dismiss" onclick="this.parentElement.remove()"><span class="screen-reader-text"><?php echo esc_html__('Dismiss this notice.', 'woocommerce')  ?></span></button></div>');
                    }
                });
            </script>
            <?php
    ...
    {
        parent::lyra_admin_head_script();
        ?>
        <script type="text/javascript">
            //<!--
            function lyraShowSubscriptionsWarningMessage() {
                var subscriptions = jQuery('#<?php echo esc_attr($this->get_field_key('subscriptions')); ?> option:selected').val();
                if ((<?php echo LyraTools::is_plugin_not_active('woocommerce-subscriptions/woocommerce-subscriptions.php'); ?>) && (subscriptions === 'wc-subscriptions') &&
                    ! confirm('<?php echo sprintf(__('Warning! %s plugin must be installed and activated for the subscription payment method to work.', 'woo-lyra-payment'), 'WooCommerce Subscriptions')?>')) {
                    lyraResetSubscriptionsField();
                } else if ((<?php echo LyraTools::is_plugin_not_active('subscriptio/subscriptio.php'); ?>) && (subscriptions === 'subscriptio') &&
                    ! confirm('<?php echo sprintf(__('Warning! %s plugin must be installed and activated for the subscription payment method to work.', 'woo-lyra-payment'), 'Subscriptio')?>')) {
                    lyraResetSubscriptionsField();
                } else if ((subscriptions === 'custom') &&
                    ! confirm('<?php echo __('Warning! You have to implement a subscriptions adapter for our plugin.', 'woo-lyra-payment')?>')) {
                    lyraResetSubscriptionsField();
                }
            }

            function lyraResetSubscriptionsField() {
...
<?php
    ...
    public function admin_options()
    {
        if ($this->get_option('subscriptions') === 'wc-subscriptions'
            && LyraTools::is_plugin_not_active('woocommerce-subscriptions/woocommerce-subscriptions.php') === 'true') {
            echo '<div class="inline error"><p><strong>' . sprintf(__('Warning! %s plugin must be installed and activated for the subscription payment method to work.', 'woo-lyra-payment'), 'WooCommerce Subscriptions') . '</strong></p></div>';
        }
    ...
    public function process_payment($order_id)
    {
        ...
        if (isset($_POST['lyrasubscription_old_pm'])) {
            set_transient($this->id . '_change_payment_' . $order_id, $_POST['lyrasubscription_old_pm']);
        }
    ...
    public function process_admin_options()
    {
        ...
        $wcs_subscription_settings = get_option('woocommerce_lyrawcssubscription_settings', null);
        ...
        WC_Admin_Settings::add_error(sprintf(__('This method cannot be enabled. You have to disable "%s" method first.', 'woo-lyra-payment'),
            self::GATEWAY_NAME . ' - ' . __('Subscription payment with WooCommerce Subscriptions', 'woo-lyra-payment')));

        $settings['enabled'] = 'no';
        update_option('woocommerce_' . $this->id . '_settings', $settings);
    }

    public function lyra_init()
    {
        parent::lyra_init();
