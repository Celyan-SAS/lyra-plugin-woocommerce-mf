...
class WC_Gateway_LyraRegroupedOther extends WC_Gateway_LyraStd
{
    public function __construct($regroup = true)
    {
        $this->id = 'lyraregroupedother';
        $this->icon = apply_filters('woocommerce_' . $this->id . '_icon', WC_LYRA_PLUGIN_URL . 'assets/images/other.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Other payment means', 'woo-lyra-payment');
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields['enabled']['description'] = __('Enables / disables this payment method.', 'woo-lyra-payment');

        $this->form_fields['title']['default'] = __('Other payment means', 'woo-lyra-payment');
        ...
        if (version_compare($woocommerce->version, '2.3.0', '>=')) {
            $this->form_fields['regroup_enabled'] = array(
                'title' => __('Regroup payment means', 'woo-lyra-payment'),
                'label' => __('Enable / disable', 'woo-lyra-payment'),
                'type' => 'checkbox',
                'default' => 'no',
                'description' => __('If this option is enabled, all the payment means added in this section will be displayed within the same payment submodule.', 'woo-lyra-payment')
            );
        }

        $smartform_mode = LyraTools::is_embedded_payment();
        ...
        $descr = sprintf(__($payment_options_desc,
            'woo-lyra-payment'), 'Lyra Expert', 'Lyra Collect');
        ...
        $columns['payment_mean'] = array(
            'title' => __('Means of payment', 'woo-lyra-payment'),
            'width' => '250px'
        );
        ...
        $columns['countries'] = array(
            'title' => __('Countries', 'woo-lyra-payment'),
            'width' => '175px',
        );
        ...
        if ($smartform_mode) {
            $columns['integrated_mode'] = array(
                'title' => __('Embedded mode', 'woo-lyra-payment'),
                'width' => '92px',
            );
        }

        $columns['send_cart_data'] = array(
            'title' => __('Cart data', 'woo-lyra-payment'),
            'width' => '92px',
        );

        $this->form_fields['payment_means'] = array(
            'title' => __('Payment means', 'woo-lyra-payment'),
            'type' => 'table',
            'columns' => $columns,
            'description' => $descr
        );
        ...
        $culumns['code'] = array(
            'title' => __('Code', 'woo-lyra-payment'),
            'width' => '150px'
        );
        ...
        $this->form_fields['extra_payment_means'] = array(
            'title' => __('Add payment means', 'woo-lyra-payment'),
            'type' => 'table',
            'columns' => $culumns,
            'description' => $descr
        );
    ...
    {
        return __('Method title to display on payment means page. Used only if &laquo; Regroup payment means &raquo; option is enabled.', 'woo-lyra-payment');
    }

    protected function get_method_description_field_description()
    {
        return __('This controls the description which the user sees during checkout. Used only if &laquo; Regroup payment means &raquo; option is enabled.', 'woo-lyra-payment');
    }
    ...
    public function lyra_admin_head_script()
    {
        ...
        //<!--
                    ...
                    switch (attr) {
                        ...
                        case 'countries':
                            ...
                            optionLine += '<div><select style="display:none; width: ' + width + 'px; height: 150px; background-color: white; padding: 2px;" name="' + inputName + '[]" id="' + inputName +
                                         '" multiple="multiple" onblur="javascript:lyraDisplayMultiSelect(\'' + inputName + '\'); lyraDisplayLabel(\'' + inputName + '\');">';
                            optionLine += '<?php
                                                ...
                                                if (labelValue == '') {
                                                    labelValue = '<?php echo __('Click to add countries.', 'woo-lyra-payment')?>';
                                                }

                            optionLine += '</select><label style="width:100%;" id="label_' + inputName + '" onclick="javascript:lyraDisplayMultiSelect(\'' + inputName + '\');" >' + labelValue + '</label></div>';
                            optionLine += '</td>';
                            break;
                        ...
                        case 'integrated_mode':
                            if (smartformActivated) {
                                ...
                                optionLine += '<input type="checkbox" name="' + inputName + '" id="' + inputName + '" ' + checked + ' onclick="javascript:lyraActivateFields(\'' + inputName + '\', \'' + orderedRecord.validation_mode + '\', \'' + orderedRecord.capture_delay + '\');" >';
                                optionLine += '</td>';
                            }
                        ...
                        case 'send_cart_data':
                            optionLine += '<td style="padding: 0px;">';
                            optionLine += '<select style="width: ' + width + 'px;" name="' + inputName + '" id="' + inputName + '">';
                            optionLine += '<?php $options = array('n' => __('No', 'woo-lyra-payment'), 'y' => __('Yes', 'woo-lyra-payment'));
            ...
            function lyraActivateFields(checkboxId, validationMode, captureDelay) {
            ...
            function lyraDisplayLabel(selectId) {
                var select = document.getElementById(selectId);
                var label = document.getElementById('label_' + selectId);
            ...
            function getLabelText(select) {
                ...
                if (! labelText) {
                    labelText = '<?php echo __('Click to add countries.', 'woo-lyra-payment')?>';
                }
...
<?php
            ...
            if (($option['amount_min'] && (! is_numeric($option['amount_min']) || $option['amount_min'] < 0))
                ...
                if (in_array($option['payment_mean'], $used_cards)) {
                    ...
                    if (! $option['label']) {
                        ...
                        $value[$code]['label'] = sprintf(__('Payment with %s', 'woo-lyra-payment'), $cards[$option['payment_mean']]);
                    }
    ...
    protected function lyra_fill_request($order)
    {
        ...
        if (isset($option['capture_delay']) && is_numeric($option['capture_delay'])) {
            $this->lyra_request->set('capture_delay', $option['capture_delay']);
        }

        if (isset($option['validation_mode']) && ($option['validation_mode'] !== '-1')) {
            $this->lyra_request->set('validation_mode', $option['validation_mode']);
        }
