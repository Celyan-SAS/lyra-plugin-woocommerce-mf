...
class WC_Gateway_LyraStd extends WC_Gateway_Lyra
{
    const ALL_COUNTRIES = '1';
    const SPECIFIC_COUNTRIES = '2';

    protected $lyra_countries = array();
    protected $lyra_currencies = array();
    protected $lyra_other_payment_methods;

    public function __construct()
    {
        $this->id = 'lyrastd';
        $this->icon = apply_filters('woocommerce_lyrastd_icon', WC_LYRA_PLUGIN_URL . 'assets/images/lyra.png');
        $this->has_fields = true;
        $this->method_title = self::GATEWAY_NAME . ' - ' . __('Standard payment', 'woo-lyra-payment');

        // To get payment means in case of Smartform.
        $this->lyra_other_payment_methods = new WC_Gateway_LyraRegroupedOther(false);
        ...
        add_action('woocommerce_api_wc_gateway_' . $this->id, array($this, 'lyra_generate_iframe_form'));

        // Return from REST payment action.
        add_action('woocommerce_api_wc_gateway_lyra_rest', array($this, 'lyra_rest_return_response'));

        // Notification from REST payment action.
        add_action('woocommerce_api_wc_gateway_lyra_notify_rest', array($this, 'lyra_rest_notify_response'));

        // Rest payment generate token.
        add_action('woocommerce_api_wc_gateway_lyra_form_token', array($this, 'lyra_refresh_form_token'));

        // Set payment method in for orders created from BO.
        add_action('woocommerce_api_wc_gateway_lyra_process_order', array($this, 'process_order_payment'));

        // Adding JS to load REST libs.
        add_action('wp_head', array($this, 'lyra_rest_head_script'));
    }

    public function lyra_rest_head_script()
    {
        if (LyraTools::is_embedded_payment(false) && $this->is_available()) {
            $lyra_pub_key = $this->testmode ? $this->get_general_option('test_public_key') : $this->get_general_option('prod_public_key');

            $locale = get_locale() ? substr(get_locale(), 0, 2) : null;
            if (! $locale || ! LyraApi::isSupportedLanguage($locale)) {
                $locale = $this->settings['language'];
            }

            $language_iso_code = $locale;
            $return_url = add_query_arg('wc-api', 'WC_Gateway_Lyra_Rest', home_url('/'));
            $custom_placeholders = '';
            ...
            // Custom theme.
            $lyra_std_rest_theme = $this->settings['rest_theme'];
            $lyra_static_url = $this->get_general_option('static_url', self::STATIC_URL);

            $lyra_display = 'flex';
            if (LyraTools::is_embedded_payment() && ($this->get_option('rest_popin') == 'yes')) {
                $lyra_display = 'none !important';
            }
            ?>
                <script>
                    var LYRA_LANGUAGE = "<?php echo $language_iso_code; ?>"
                </script>
                <script src="<?php echo $lyra_static_url; ?>js/krypton-client/V4.0/stable/kr-payment-form.min.js"
                        kr-public-key="<?php echo $lyra_pub_key; ?>"
                ...
                <link rel="stylesheet" href="<?php echo $lyra_static_url; ?>js/krypton-client/V4.0/ext/<?php echo $lyra_std_rest_theme;?>-reset.css">
                <script src="<?php echo $lyra_static_url; ?>js/krypton-client/V4.0/ext/<?php echo $lyra_std_rest_theme;?>.js"></script>

                <style>
                    #lyrastd_rest_wrapper button.kr-popin-button {
                    ...
                    .kr-smart-form .kr-smart-button {
                        display: <?php echo $lyra_display; ?>;
                    }
                </style>
            <?php

            // Load REST script.
            if (! wp_script_is('lyra-rest-js', 'registered')) {
                wp_register_script('lyra-rest-js', WC_LYRA_PLUGIN_URL . 'assets/js/rest.js');
                wp_enqueue_script('lyra-rest-js');
            }
        }
    }
    ...
    {
        if ($this->id === 'lyrastd' && LyraTools::is_embedded_payment(false) && ($this->get_option('rest_popin') !== 'yes')) {
            return '';
        } else {
    ...
    {
        if (empty($this->lyra_countries)) {
            return $countries;
        } else {
            $allowed_countries = array();
            foreach ($this->lyra_countries as $code) {
    ...
    public function init_form_fields()
    {
        ...
        $this->form_fields = array(
            ...
            'module_settings' => array(
                'title' => __('MODULE SETTINGS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'enabled' => array(
                'title' => __('Activation', 'woo-lyra-payment'),
                'label' => __('Enable / disable', 'woo-lyra-payment'),
                'type' => 'checkbox',
                'default' => 'yes',
                'description' => __('Enables / disables standard payment.', 'woo-lyra-payment')
            ),
            'title' => array(
                'title' => __('Title', 'woo-lyra-payment'),
                'type' => 'text',
                'description' => $this->get_method_title_field_description(),
                'default' => __('Payment by credit card', 'woo-lyra-payment')
            ),
            'description' => array(
                'title' => __('Description', 'woo-lyra-payment'),
                'type' => 'textarea',
                'description' => $this->get_method_description_field_description(),
                'default' => __('You will enter payment data after order confirmation.', 'woo-lyra-payment'),
                'css' => 'width: 35em;'
            ),

            // Amount restrictions.
            'restrictions' => array(
                'title' => __('RESTRICTIONS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'allows_specific' => array(
                'custom_attributes' => array(
                    'onchange' => 'lyraUpdateSpecificCountriesDisplay()'
                ),
                'title' => __('Restrict to some countries', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => '1',
                'options' => array(
                    self::ALL_COUNTRIES => __('All allowed countries', 'woo-lyra-payment'),
                    self::SPECIFIC_COUNTRIES => __('Specific countries', 'woo-lyra-payment')
                ),
                'class' => 'wc-enhanced-select',
                'description' => __('Buyer\'s billing countries in which this payment method is available.', 'woo-lyra-payment')
            ),
            'specific_countries' => array(
                'title' => __('Authorized countries', 'woo-lyra-payment'),
                'type' => 'multiselect',
                'default' => '',
            ...
            'amount_min' => array(
                'title' => __('Minimum amount', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => '',
                'description' => __('Minimum amount to activate this payment method.', 'woo-lyra-payment')
            ),
            'amount_max' => array(
                'title' => __('Maximum amount', 'woo-lyra-payment'),
                'type' => 'text',
                'default' => '',
                'description' => __('Maximum amount to activate this payment method.', 'woo-lyra-payment')
            ),
            ...
            'capture_delay' => array(
                ...
                'type' => 'text',
                'default' => '',
                'description' => sprintf(__('The number of days before the bank capture. Enter value only if different from %s general configuration.', 'woo-lyra-payment'), self::GATEWAY_NAME)
            ),
            ...
            'payment_cards' => array(
                'title' => __('Card Types', 'woo-lyra-payment'),
                'type' => 'multiselect',
                'default' => array(),
                'options' => $this->get_supported_card_types(),
                'description' => __('The card type(s) that can be used for the payment. Select none to use gateway configuration.', 'woo-lyra-payment'),
                'class' => 'wc-enhanced-select'
            ),

            // Advanced options.
            'advanced_options' => array(
                'title' => __('ADVANCED OPTIONS', 'woo-lyra-payment'),
                'type' => 'title'
            ),
            'card_data_mode' => array(
                'custom_attributes' => array(
                    'onchange' => 'lyraUpdateRestFieldDisplay(false)'
                ),
                'title' => __('Payment data entry mode', 'woo-lyra-payment'),
                'type' => 'select',
                'default' => 'DEFAULT',
                'options' => array(
                    'DEFAULT' => __('Card data entry on payment gateway', 'woo-lyra-payment'),
                    'MERCHANT' => __('Card type selection on merchant site', 'woo-lyra-payment'),
                    'IFRAME' => __('Payment page integrated to checkout process (iframe)', 'woo-lyra-payment'),
                ),
                'description' => __('Select how the payment data will be entered by buyer. Think to update payment method description to match your selected mode.', 'woo-lyra-payment'),
                'class' => 'wc-enhanced-select'
            )
        );

        // Add REST fields if available for payment.
        if ($lyra_plugin_features['embedded']) {
            $this->form_fields['card_data_mode']['options']['REST'] = __('Embedded payment fields on merchant site (REST API)', 'woo-lyra-payment');
            if ($lyra_plugin_features['smartform']) {
                $this->form_fields['card_data_mode']['options']['SMARTFORM'] = __('Embedded Smartform on merchant site (REST API)', 'woo-lyra-payment');
                $this->form_fields['card_data_mode']['options']['SMARTFORMEXT'] = __('Embedded Smartform extended on merchant site with logos (REST API)', 'woo-lyra-payment');
                $this->form_fields['card_data_mode']['options']['SMARTFORMEXTNOLOGOS'] = __('Embedded Smartform extended on merchant site without logos (REST API)', 'woo-lyra-payment');
            }
        ...
        $this->form_fields['payment_by_token'] = array(
            'custom_attributes' => array(
                'onchange' => 'lyraUpdatePaymentByTokenField()',
            ),
            'title' => __('Payment by token', 'woo-lyra-payment'),
            'type' => 'select',
            'default' => '0',
            'options' => array(
                '1' => __('Yes', 'woo-lyra-payment'),
                '0' => __('No', 'woo-lyra-payment')
            ),
            'description' => sprintf(__('The payment by token allows to pay orders without re-entering bank data at each payment. The "Payment by token" option should be enabled on your %s store to use this feature.', 'woo-lyra-payment'), self::GATEWAY_NAME),
            'class' => 'wc-enhanced-select'
        );
    ...
    protected function get_rest_fields()
    {
        ...
        $this->form_fields['rest_customization'] = array(
            'title' => __('CUSTOMIZATION', 'woo-lyra-payment'),
            'type' => 'title',
        );

        $this->form_fields['rest_popin'] = array(
            'title' => __('Display in a pop-in', 'woo-lyra-payment'),
            'label' => __('Enable / disable', 'woo-lyra-payment'),
            'type' => 'checkbox',
            'default' => 'no',
            'description' => __('If enabled, the selected payment mode will be displayed in a pop-in.', 'woo-lyra-payment')
        );

        $this->form_fields['rest_theme'] = array(
            'title' => __('Theme', 'woo-lyra-payment'),
            'type' => 'select',
            'default' => 'neon',
            'options' => array(
                'neon' => 'Neon',
            ...
            'description' => __('Select a theme to use to display the embedded payment fields or the Smartform.', 'woo-lyra-payment'),
            'class' => 'wc-enhanced-select'
        );

        $this->form_fields['smartform_compact_mode'] = array(
            'title' => __('Compact mode', 'woo-lyra-payment'),
            'label' => __('Enable / disable', 'woo-lyra-payment'),
            'type' => 'checkbox',
            'default' => 'no',
            'description' => __('If enabled, the Smartform will be displayed in a compact mode.', 'woo-lyra-payment')
        );

        $this->form_fields['smartform_grouping_threshold'] = array(
            'title' => __('Payment means grouping threshold', 'woo-lyra-payment'),
            'type' => 'text',
            'description' => __('Number from which payment means will be grouped.', 'woo-lyra-payment')
        );

        $this->form_fields['rest_placeholder'] = array(
            'title' => __('Custom fields placeholders', 'woo-lyra-payment'),
            ...
            'description' => __('Texts to use as placeholders for embedded payment fields, Smartform or Smartform extended.', 'woo-lyra-payment')
        );

        $this->form_fields['rest_register_card_label'] = array(
            'title' => __('Register card label', 'woo-lyra-payment'),
            'type' => 'text',
            'default' => __('Register my card', 'woo-lyra-payment'),
            'description' => __('Label displayed to invite buyers to register their card data.', 'woo-lyra-payment')
        );

        $this->form_fields['rest_attempts'] = array(
            'title' => __('Payment attempts number for cards', 'woo-lyra-payment'),
            'type' => 'text',
            'description' => __('Maximum number of payment by cards retries after a failed payment (between 0 and 2). If blank, the gateway default value is 2.', 'woo-lyra-payment')
        );
    ...
    public function generate_placeholder_table_html($key, $data)
    {
        ...
        $html .= '<tr class="lyra-placeholder">
                   <td>
                     <label style="color: #23282d; font-weight: 500;" for="' . $field_name . '_pan">' . __('Card number', 'woo-lyra-payment') . '</label>
                   </td>
                   <td>
        ...
        $html .= '<tr class="lyra-placeholder">
                   <td>
                     <label style="color: #23282d; font-weight: 500;" for="' . $field_name . '_expiry">' . __('Expiry date', 'woo-lyra-payment') . '</label>
                   </td>
                   <td>
        ...
        $html .= '<tr class="lyra-placeholder">
                   <td>
                     <label style="color: #23282d; font-weight: 500;" for="' . $field_name . '_cvv">' . __('CVV', 'woo-lyra-payment') . '</label>
                   </td>
                   <td>
    ...
    public function lyra_admin_head_script()
    {
        ...
        //<!--
            jQuery(function() {
                lyraUpdateSpecificCountriesDisplay();
            });

            function lyraUpdateSpecificCountriesDisplay() {
            ...
            jQuery(document).ready(function() {
                lyraUpdateRestFieldDisplay();
            });

            function lyraUpdateRestFieldDisplay(ignoreIframe = true) {
                ...
                if (jQuery.inArray(cardDataMode, ['REST', 'SMARTFORM', 'SMARTFORMEXT', 'SMARTFORMEXTNOLOGOS']) != -1) {
                    ...
                    if (! ignoreIframe) {
                        if ((cardDataMode === 'IFRAME') &&
                            ! confirm('<?php echo __('Warning, some payment means are not compatible with an integration by iframe. Please consult the documentation for more details.', 'woo-lyra-payment')?>')) {
    ...
    {
        if (! empty($this->lyra_currencies)) {
            return in_array(get_woocommerce_currency(), $this->lyra_currencies);
        }
    ...
    protected function is_available_for_country()
    {
        ...
        if ($this->get_option('allows_specific') === self::ALL_COUNTRIES) {
            return empty($this->lyra_countries) || in_array($country, $this->lyra_countries);
        }
    ...
    protected function is_available_for_subscriptions()
    {
        ...
        if (class_exists('WC_Gateway_LyraSubscription')) {
            ...
            $handler = is_array($settings) && isset($settings['subscriptions']) ? $settings['subscriptions'] :
                WC_Gateway_LyraSubscription::SUBSCRIPTIONS_HANDLER;
            $subscriptions_handler = Lyra_Subscriptions_Loader::getInstance($handler);
    ...
    public function get_payment_fields()
    {
        ...
        $process_order_url = add_query_arg('wc-api', 'WC_Gateway_Lyra_Process_Order', home_url('/'));
        ...
        switch ($option) {
            ...
            case 'IFRAME':
                ...
                $html .= '<div>
                         <iframe name="lyra_iframe" id="lyra_iframe" src="' . $src . '" style="display: none;">
                         </iframe>';
                ...
                if ($order = self::order_created_from_bo()) {
                    ...
                    $html .= "jQuery('#order_review').on('submit', function(e) {
                        ...
                        jQuery('#lyra_iframe').attr('src', '$link');
                        jQuery.ajax({
                            method: 'POST',
                            url: '" . $process_order_url . "',
                            data: { 'lyra_order_id': ". self::get_order_property($order, 'id'). " },
                    ...
                    $html .= "jQuery('input[type=\"radio\"][name=\"payment_method\"][value!=\"" . $this->id . "\"]').click(function() {
                                ...
                                jQuery('#lyra_iframe').hide();
                                jQuery('#lyra_iframe').attr('src', '" . $src . "');
                    });</script>";
                    break;
                }
                                ...
                                window.fetch = (...args) => {
                                    ...
                                    return Promise.resolve(_fetch.apply(window, args))
                                      .then(async (response) => {
                                        ...
                                        if (data.indexOf('payment_method=lyrastd') == -1) {
                                            return response;
                                        }
            ...
            case 'SMARTFORMEXTNOLOGOS':
                ...
                if ($order = self::order_created_from_bo()) {
                    $html .= '<script type="text/javascript">';
                    $html .= "jQuery('#order_review').on('submit', function(e) {
                        if (! jQuery('#payment_method_lyrastd').is(':checked')) {
                            return true;
                        }
                        e.preventDefault();
                        // Unblock screen.
                ...
                $form_token_url = add_query_arg('wc-api', 'WC_Gateway_Lyra_Form_Token', home_url('/'));
                ...
                $html .= "jQuery('form.checkout').on('checkout_place_order_" . $this->id . "', function() {
                                ...
                                jQuery.ajaxPrefilter(function(options, originalOptions, jqXHR) {
                                    ...
                                    if (options.data.indexOf('payment_method=lyrastd') == -1) {
                                        return;
                                    }
                                    ...
                                    options.success = function(data, status, jqXHR) {
                                        ...
                                        if (newData === savedData) {
                                            ...
                                            var useIdentifier = jQuery('#lyra_use_identifier').length && jQuery('#lyra_use_identifier').val();
                                            savedData = newData;
                                            jQuery.ajax({
                                                method: 'POST',
                                                url: '" . $form_token_url . "',
                                                ...
                                                success: function(data) {
                                                    ...
                                                    KR.setFormConfig({" . $sf_form_config_js . "language: LYRA_LANGUAGE, formToken: parsed.formToken}).then(function(v) {
                                                        var KR = v.KR;
                                                        if (registerCard) {
        ...
        if ($can_pay_by_alias) {
            ...
            if ($force_redir) {
                $html = '<input type="hidden" name="lyra_force_redir" value="true">';
            }

            if ($this->id === 'lyramulti') {
                return $html;
            }
    ...
    protected function can_use_alias($cust_id, $verify_identifier = false)
    {
        ...
        return (! $verify_identifier ||
            ((LyraTools::has_checkout_block() || self::order_created_from_bo() || ! empty($_GET['wc-ajax'])) && $this->check_identifier($cust_id, $this->id)));
    }
    ...
    protected function payment_by_alias_view($payment_fields, $force_redir)
    {
        ...
        $embedded_fields = LyraTools::is_embedded_payment(false) && ($this->get_option('rest_popin') !== 'yes') && ! empty($payment_fields);
        ...
        $html .= '    </li>
                  ...
                  <li style="list-style-type: none;" class="block ' . $this->id . '-id-block">
                      <input id="lyra_use_identifier" type="hidden" value="true" name="lyra_use_identifier">
                      <span>' .
                          sprintf(__('You will pay with your stored means of payment %s', 'woo-lyra-payment'), $saved_masked_pan)
                          . ' (<a href="' . esc_url(wc_get_account_endpoint_url($this->get_option('woocommerce_saved_cards_endpoint', 'ly_saved_cards'))) . '">' . __('manage your payment means', 'woo-lyra-payment') . '</a>).
                      </span>
                  </li>';
        ...
        $html .= '</ul>
              ...
              <ul class="lyrastd-view-bottom" style="margin-left: 0; margin-top: 0;">
                  <li style="list-style-type: none; margin: 15px 0px;" class="block ' . $this->id . '-cc-block ' . $this->id . '-id-block">
                      <span>' . __('OR', 'woo-lyra-payment') . '</span>
                  </li>

                  <li style="list-style-type: none;" class="block ' . $this->id . '-cc-block">
                      <a href="javascript: void(0);" onclick="lyraUpdatePaymentBlock(true)">' . __('Click here to pay with your registered means of payment.', 'woo-lyra-payment') . '</a>
                  </li>

                  <li style="list-style-type: none;" class="block ' . $this->id . '-id-block">
                      <a href="javascript: void(0);" onclick="lyraUpdatePaymentBlock(false)">' . __('Click here to pay with another means of payment.', 'woo-lyra-payment') . '</a>
                  </li>
              </ul>';

        $html .= '<script type="text/javascript">
                  function lyraUpdatePaymentBlock(useIdentifier) {
                      jQuery("ul.' . $this->id . '-view-top li.block").hide();
                      jQuery("ul.lyrastd-view-bottom li.block").hide();
                      var blockName = useIdentifier ? "id" : "cc";
                      jQuery("li.' . $this->id . '-" + blockName + "-block").show();
                      if (typeof lyraUpdateFormToken === "function") {
    ...
    private function rest_payment_fields_view($use_identifier)
    {
        ...
        $img_url = WC_LYRA_PLUGIN_URL . 'assets/images/loading.gif';
        $popin_attr = '';
        $cards_form = 'false';
        $html = '';
        $sfExpanded = '';
        ...
        if (LyraTools::is_embedded_payment(false)) {
            if ($this->get_option('rest_popin') === 'yes') {
                $popin_attr = 'kr-popin';
            }
            ...
            if (LyraTools::is_embedded_payment()) {
                $groupingThreshold = $this->get_option('smartform_grouping_threshold');
        ...
        set_transient('lyra_hide_smartbutton_' . wp_get_session_token(), $hide_smart_button_popin);

        $html .= '<div id="lyrastd_rest_wrapper"></div>';
        $html .= '<script type="text/javascript">';
        ...
        $jsVars .= 'window.LYRA_KR_MODE = "' . $krMode . ' ";';
        $jsVars .= 'window.LYRA_SINGLE_PAYMENT_BUTTON_MODE = "' . $single_payment_button . '";';
        $jsVars .= 'window.LYRA_SF_EXTENDED_MODE = "' . $sfExpanded . '";';
        $jsVars .= 'window.LYRA_POPIN_ATTR = "' . $popin_attr . '";';
        $jsVars .= 'window.LYRA_SF_STYLE = new Boolean(' . (empty($sfStyle) ? 'false' : 'true') . ');';
        $jsVars .= 'window.LYRA_CARDS_FORM = new Boolean(' . $cards_form . ');';
        $jsVars .= 'window.LYRA_IMG_URL = "' . $img_url . '";';
        $jsVars .= 'window.LYRA_SF_COMPACT_MODE = new Boolean(' . $compact_mode . ');';
        $jsVars .= 'window.LYRA_GROUPING_THRESHOLD = "' . (is_numeric($groupingThreshold) ? $groupingThreshold : 'false') . '";';
        $jsVars .= 'window.LYRA_HIDE_SINGLE_BUTTON = new Boolean(' . $hide_single_payment_button . ');';

        set_transient('lyra_js_vars_' . wp_get_session_token(), $jsVars);
    ...
    private function get_smartform_cards()
    {
        ...
        if (LyraTools::is_embedded_payment() && $this->lyra_other_payment_methods->is_available_ignoring_regroup()) {
            $available_options = $this->lyra_other_payment_methods->get_available_options_for_smartform();
    ...
    private function get_temporary_form_token($use_identifier = false)
    {
        ...
        $last_token_data = get_transient('lyra_token_data_' . ($use_identifier ? 'identifier_' : '') . wp_get_session_token());
        $last_token = get_transient('lyra_token_' . ($use_identifier ? 'identifier_' : '') . wp_get_session_token());
        ...
        try {
            $client = new LyraRest($this->get_general_option('rest_url'), $this->get_general_option('site_id'), $key);
            ...
            if ($result['status'] != 'SUCCESS') {
                ...
                set_transient('lyra_token_data_' . ($use_identifier ? 'identifier_' : '') . wp_get_session_token(), $token_data);
                set_transient('lyra_token_' . ($use_identifier ? 'identifier_' : '') . wp_get_session_token(), $return);
            }
        } catch (Exception $e) {
    ...
    private function get_form_token($order, $use_identifier = false)
    {
        ...
        $order_id = $this->get_escaped_var($this->lyra_request, 'order_id');
        $currency = LyraApi::findCurrencyByAlphaCode(get_woocommerce_currency());
        ...
        $params = array(
            'orderId' => $order_id,
            'customer' => array(
                'email' => $this->get_escaped_var($this->lyra_request, 'cust_email'),
                'reference' => $this->get_escaped_var($this->lyra_request, 'cust_id'),
                'billingDetails' => array(
                    'language' => $this->get_escaped_var($this->lyra_request, 'language'),
                    'title' => $this->get_escaped_var($this->lyra_request, 'cust_title'),
                    'firstName' => $this->get_escaped_var($this->lyra_request, 'cust_first_name'),
                    'lastName' => $this->get_escaped_var($this->lyra_request, 'cust_last_name'),
                    'category' => $this->get_escaped_var($this->lyra_request, 'cust_status'),
                    'address' => $this->get_escaped_var($this->lyra_request, 'cust_address'),
                    'zipCode' => $this->get_escaped_var($this->lyra_request, 'cust_zip'),
                    'city' => $this->get_escaped_var($this->lyra_request, 'cust_city'),
                    'state' => $this->get_escaped_var($this->lyra_request, 'cust_state'),
                    'phoneNumber' => $this->get_escaped_var($this->lyra_request, 'cust_phone'),
                    'country' => $this->get_escaped_var($this->lyra_request, 'cust_country'),
                    'identityCode' => $this->get_escaped_var($this->lyra_request, 'cust_national_id'),
                    'streetNumber' => $this->get_escaped_var($this->lyra_request, 'cust_address_number'),
                    'district' => $this->get_escaped_var($this->lyra_request, 'cust_district'),
                    'status' => $this->get_escaped_var($this->lyra_request, 'cust_status')
                ),
                'shippingDetails' => array(
                    'firstName' => $this->get_escaped_var($this->lyra_request, 'ship_to_first_name'),
                    'lastName' => $this->get_escaped_var($this->lyra_request, 'ship_to_last_name'),
                    'category' => $this->get_escaped_var($this->lyra_request, 'ship_to_status'),
                    'address' => $this->get_escaped_var($this->lyra_request, 'ship_to_street'),
                    'address2' => $this->get_escaped_var($this->lyra_request, 'ship_to_street2'),
                    'zipCode' => $this->get_escaped_var($this->lyra_request, 'ship_to_zip'),
                    'city' => $this->get_escaped_var($this->lyra_request, 'ship_to_city'),
                    'state' => $this->get_escaped_var($this->lyra_request, 'ship_to_state'),
                    'phoneNumber' => $this->get_escaped_var($this->lyra_request, 'ship_to_phone_num'),
                    'country' => $this->get_escaped_var($this->lyra_request, 'ship_to_country'),
                    'deliveryCompanyName' => $this->get_escaped_var($this->lyra_request, 'ship_to_delivery_company_name'),
                    'shippingMethod' => $this->get_escaped_var($this->lyra_request, 'ship_to_type'),
                    'shippingSpeed' => $this->get_escaped_var($this->lyra_request, 'ship_to_speed')
                )
            ),
            'transactionOptions' => array(
                'cardOptions' => array(
                    'paymentSource' => 'EC',
                    'captureDelay' => $this->get_escaped_var($this->lyra_request, 'capture_delay') // In case of Smartform, only payment means supporting capture delay will be shown.
                )
            ),
            'contrib' => $this->get_escaped_var($this->lyra_request, 'contrib'),
            'strongAuthentication' => $strong_auth,
            'currency' => $currency->getAlpha3(),
            'amount' => $this->get_escaped_var($this->lyra_request, 'amount'),
            'metadata' => array(
        ...
        $validationMode = $this->lyra_request->get('validation_mode');
        ...
        if ($use_identifier) {
            ...
            $this->log('Customer ' . $this->lyra_request->get('cust_email') . ' will be asked for card data registration.');
            $params['formAction'] = 'ASK_REGISTER_PAY';
        }
        ...
        try {
            $client = new LyraRest($this->get_general_option('rest_url'), $this->lyra_request->get('site_id'), $key);
            $result = $client->post('V4/Charge/CreatePayment', json_encode($params));
    ...
    public function lyra_refresh_form_token()
    {
        global $woocommerce;
    ...
    public function process_payment($order_id)
    {
        ...
        // Set flag about use of saved identifier.
        if (isset($_POST['lyra_use_identifier']) || isset($_COOKIE['lyra_use_identifier'])) {
            $use_identifier = isset($_POST['lyra_use_identifier']) ? $_POST['lyra_use_identifier'] : $_COOKIE['lyra_use_identifier'];
            set_transient($this->id . '_use_identifier_' . $order_id, $use_identifier === 'true');
        }

        $order = new WC_Order($order_id);
        $force_redir = isset($_POST['lyra_force_redir']) || (isset($_COOKIE['lyra_force_redir']) || ! empty($_COOKIE['lyra_force_redir']));

        // If ($_POST || $_COOKIE)['lyra_force_redir'] is set, force payment by redirection.
        if (($this->id === 'lyrastd') && (LyraTools::is_embedded_payment(false) && ! $force_redir || $this->get_option('card_data_mode') === 'IFRAME')) {
            $return = array(
                'result' => 'success',
                'messages' => '<div></div>'
            );
    ...
    public function process_order_payment() {
        ...
        if (isset($_POST['lyra_order_id'])) {
            $order = new WC_Order($_POST['lyra_order_id']);
     ...
     **/
    public function lyra_generate_form($order_id)
    {
        global $woocommerce;
        ...
        echo __('Please wait, you will be redirected to the payment gateway.', 'woo-lyra-payment');
        echo '</div>';
        echo '<br />';
        echo '<p>' . __('If nothing happens in 10 seconds, please click the button below.', 'woo-lyra-payment') . '</p>';

        $this->lyra_fill_request($order);

        // Log data that will be sent to payment gateway.
        $this->log('Data to be sent to payment gateway : ' . print_r($this->lyra_request->getRequestFieldsArray(true /* To hide sensitive data. */), true));

        $form = '<form action="' . esc_url($this->lyra_request->get('platform_url')) . '" method="post" id="' . $this->id . '_payment_form">';
        $form .= '  <input type="submit" class="button-alt" id="' . $this->id . '_payment_form_submit" value="' . sprintf(__('Pay via %s', 'woo-lyra-payment'), self::GATEWAY_NAME).'">';
        $form .= '  <a class="button cancel" href="' . esc_url($order->get_cancel_order_url()) . '">' . __('Cancel order &amp; restore cart', 'woo-lyra-payment') . '</a>';
        $form .= $this->lyra_request->getRequestHtmlFields();
        $form .= '</form>';

        $form .= '<script type="text/javascript">';
        $form .= "function lyra_submit_form() {document.getElementById('" . $this->id . "_payment_form_submit').click();}";
        $form .= "if (window.addEventListener) {window.addEventListener('load', lyra_submit_form, false);} else if (window.attachEvent) { window.attachEvent('onload', lyra_submit_form);}";
        $form .= "</script>";

        echo $form;
    }

    public function lyra_generate_iframe_form()
    {
        global $woocommerce;
        ...
        if (isset($_COOKIE['lyra_use_identifier'])) {
            set_transient($this->id . '_use_identifier_' . $order_id, $_COOKIE['lyra_use_identifier'] === 'true');
        }
        ...
        $this->lyra_request->set('theme_config', '3DS_LOGOS=false;');

        $this->lyra_request->set('action_mode', 'IFRAME');
        $this->lyra_request->set('redirect_enabled', '1');
        $this->lyra_request->set('redirect_success_timeout', '0');
        $this->lyra_request->set('redirect_error_timeout', '0');

        // Log data that will be sent to payment gateway.
        $this->log('Data to be sent to payment gateway : ' . print_r($this->lyra_request->getRequestFieldsArray(true /* To hide sensitive data. */), true));

        $form = '<form action="' . esc_url($this->lyra_request->get('platform_url')) . '" method="post" id="' . $this->id . '_payment_iframe_form">';
        $form .= $this->lyra_request->getRequestHtmlFields();
        $form .= '</form>';

        $form .= '<script type="text/javascript">';
        $form .= "function lyra_submit_form() {document.getElementById('" . $this->id . "_payment_iframe_form').submit();}";
        $form .= "if (window.addEventListener) { window.addEventListener('load', lyra_submit_form, false);} else if (window.attachEvent) { window.attachEvent('onload', lyra_submit_form);}";
        $form .= "</script>";
    ...
    protected function lyra_fill_request($order)
    {
        ...
        $currency = LyraApi::findCurrencyByAlphaCode($order_currency);
        if ($currency == null) {
            $this->log('The store currency (' . get_woocommerce_currency() . ') is not supported by payment gateway.');

            wp_die(sprintf(__('The store currency (%s) is not supported by %s.', 'woo-lyra-payment'), get_woocommerce_currency(), self::GATEWAY_NAME));
        }

        // Params.
        $misc_params = array(
            'amount' => $currency->convertAmountToInteger($order->get_total()),
            'contrib' => LyraTools::get_contrib(),
            ...
            // Return URLs.
            'url_return' => add_query_arg('wc-api', 'WC_Gateway_Lyra', home_url('/'))
        );

        $this->lyra_request->setFromArray($misc_params);

        $this->lyra_request->addExtInfo('order_key', self::get_order_property($order, 'order_key'));
        $this->lyra_request->addExtInfo('blog_id', $wpdb->blogid);
        $this->lyra_request->addExtInfo('session_id', wp_get_session_token());

        // VAT amount for colombian payment means.
        $this->lyra_request->set('totalamount_vat', $currency->convertAmountToInteger($order->get_total_tax()));
        ...
        $this->lyra_request->set('threeds_mpi', $threeds_mpi);

        // Detect language.
        $locale = get_locale() ? substr(get_locale(), 0, 2) : null;
        if ($locale && LyraApi::isSupportedLanguage($locale)) {
            $this->lyra_request->set('language', $locale);
        } else {
            $this->lyra_request->set('language', $this->get_general_option('language'));
        }

        // Available languages.
        $langs = $this->get_general_option('available_languages');
        if (is_array($langs) && ! in_array('', $langs)) {
            $this->lyra_request->set('available_languages', implode(';', $langs));
        }
        ...
        if (isset($this->form_fields['card_data_mode'])) {
            ...
            if ($this->get_option('card_data_mode') === 'MERCHANT') {
                ...
                $this->lyra_request->set('payment_cards', $selected_card);

                delete_transient($this->id . '_card_type_' . $order_id);
            } else {
                $cards = $this->get_option('payment_cards');
                if (is_array($cards) && ! in_array('', $cards)) {
                    $this->lyra_request->set('payment_cards', implode(';', $cards));
                }
            }
        }

        // Enable automatic redirection?
        $this->lyra_request->set('redirect_enabled', ($this->get_general_option('redirect_enabled') == 'yes') ? true : false);
        ...
        $this->lyra_request->set('redirect_success_message', $success_message);
        ...
        $this->lyra_request->set('redirect_error_message', $error_message);
        ...
        foreach ($config_keys as $key) {
            $this->lyra_request->set($key, $this->get_general_option($key));
        }

        // Check if capture_delay and validation_mode are overriden in submodules.
        if (is_numeric($this->get_option('capture_delay'))) {
            $this->lyra_request->set('capture_delay', $this->get_option('capture_delay'));
        }

        if ($this->get_option('validation_mode') !== '-1') {
            $this->lyra_request->set('validation_mode', $this->get_option('validation_mode'));
        }
    ...
    {
        $this->lyra_request->set('cust_status', 'PRIVATE');
        $this->lyra_request->set('ship_to_status', 'PRIVATE');
        ...
        if (! $selected_shipping) { // There is no shipping method.
            $this->lyra_request->set('ship_to_type', 'ETICKET');
            $this->lyra_request->set('ship_to_speed', 'EXPRESS');
        } else {
            ...
            switch ($shipping_method['type']) {
                ...
                case 'RECLAIM_IN_SHOP':
                    ...
                    $this->lyra_request->set('ship_to_delivery_company_name', preg_replace($not_allowed_chars_regex, ' ', $name));

                    break;

                default:
                    $this->lyra_request->set('ship_to_delivery_company_name', preg_replace($not_allowed_chars_regex, ' ', $carrier_name));

                    break;
            }

            $this->lyra_request->set('ship_to_type', empty($shipping_method['type']) ? null : $shipping_method['type']);
            $this->lyra_request->set('ship_to_speed', empty($shipping_method['speed']) ? null : $shipping_method['speed']);

            if ($shipping_method['speed'] === 'PRIORITY') {
                $this->lyra_request->set('ship_to_delay', empty($shipping_method['delay']) ? null : $shipping_method['delay']);
            }
        }
    }
     ...
     **/
    public function lyra_rest_return_response()
    {
        $this->lyra_manage_rest_notify_response(false);
    }

    /**
     * Check for REST notification response.
     **/
    public function lyra_rest_notify_response()
    {
        $this->lyra_manage_rest_notify_response(true);
    }

    public function lyra_manage_rest_notify_response($from_server_rest = false)
    {
        global $woocommerce ;
        ...
        if (! LyraRestTools::checkHash($raw_response, $sha_key)) {
        ...
        $response = new LyraResponse($data, null, '', '');

        parent::lyra_manage_notify_response($response, $from_server_rest, $update_on_failure, $server_rest_abandoned_payment);
    }
